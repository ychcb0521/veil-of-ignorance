...
describe('下单默认结算方式', () => {
  ...
  it('面板的结算方式只活在会话里：不落盘、不上云，刷新一律回到币本位', () => {
    const ctx = read('contexts/TradingContext.tsx');
    expect(ctx).not.toContain("usePersistedState<Record<string, SettlementMode>>('symbol_settlement_mode'");
    expect(ctx).toContain("useState<Record<string, SettlementMode>>({})");
    const sync = read('lib/simStateSync.ts');
    const excluded = /EXCLUDED_KEYS = new Set\(\[([^\]]*)\]\)/.exec(sync)?.[1] ?? '';
    expect(excluded).toContain("'symbol_settlement_mode'");
  });
