...
/**
 * 计分口径版本。
 * v4：「建/未建战役」按自然日 × 标的互斥计分。
 * v5：**删除**三项扣分——未做平仓评价 −1000、直接交易 −600、标的未建战役 −300。
 *     历史事件在迁移里被整条丢弃并重算总分（事件流即真相，Σevent = 总分）。
 */
export const EXECUTION_SCORING_VERSION = 5;

/** v5 删掉的三种事件；迁移时整条丢弃，此后也不再产出。 */
const RETIRED_EVENT_TYPES = new Set(['direct_reward', 'campaign_missing_penalty', 'review_missing_penalty']);
...
export interface ExecutionAssetState {
  ...
  reviewCount: number;
  penaltyDays: number;
  /** @deprecated v5 删除「未建战役 −300」后恒为 0；字段保留只为读老状态不炸。 */
  ...
  /**
   * 每个自然日已登记过的「直接交易」标的。
   * v5 起直接交易不再扣分，这张表只用于按标的去重计数（老状态里也有这份数据）。
   */
...
export function summarizeExecutionAssetEvents(
  ...
  // 直接交易不再产出事件,笔数从状态字段来(见 summarizeExecutionAssets 的调用方)。
...
/**
 * v5 起只认显式的 campaignSymbol。
 * 原来还有一支「从『未建战役』事件的 label 里回推标的」的兜底，
 * 那类事件已经删除，兜底随之作废。
 */
function campaignSymbolFromEvent(event: ExecutionAssetEvent): string | null {
  return normalizeExecutionSymbol(event.campaignSymbol);
}
 ...
 */
/**
 * v5 删除「标的未建战役 −300」：不再有 settleCampaignMissingPenalties。
 * 建战役仍然 +EXECUTION_CAMPAIGN_REWARD（见 recordCampaignCreated），只是不做反向惩罚。
 */
...
export function recordExecutionTrade(
  ...
  /**
   * 直接交易：v5 起**不再扣分**，也不再产出事件。
   *
   * 笔数照旧累加——决策占比要靠它，而且它是统计不是惩罚。
   * directPenalizedByDate 仍然维护：它记录的是「当日该标的已登记过」，
   * 老状态里还有这份数据，清掉会让 normalizeState 与历史流水对不上。
   */
  ...
  return {
    ...
    directPenalizedByDate: {
      ...
      [date]: symbol != null && !penalizedToday.includes(symbol)
        ? [...penalizedToday, symbol]
        : penalizedToday,
    },
  };
}
...
export function recordCampaignCreated(
  ...
  // v5：不再有「未建战役 −300」，也就没有「建了战役就撤罚退分」这回事。
  const eventsWithoutConflict = state.events;
...
export function reconcileCampaignRewards(
  ...
  // v5：没有「未建战役 −300」可撤了，只重建 +300 的奖励事件。
  const otherEvents = enrichedEvents.filter(event => event.type !== 'campaign_reward');

  const desiredCount = groups.size;
  return {
    ...state,
    points: state.points + (desiredCount - paidCount) * EXECUTION_CAMPAIGN_REWARD,
...
/**
 * v5 删除「未做平仓评价 −1000」：不再有 reviewMissingEvent / reconcileReviewMissingPenalties。
 * 复盘仍然 +EXECUTION_REVIEW_REWARD（见 recordPostTradeReviewCompleted），只是不做反向惩罚。
 */
...
export function migrateExecutionAssetScoringV2(rawState: ExecutionAssetState): ExecutionAssetState {
  ...
  const seenDirect = new Set<string>(); // `${date}|${symbol}`：当日该标的是否已数过直接交易
  let points = 0;
  let directTradeCount = 0;

  /**
   * v5 的迁移做两件事：按当前权重重新定价**保留**的事件，并把**已删除**的三种事件整条丢弃。
   *
   * 丢弃而不是记 0 分：事件流是流水，一条 0 分的「未做平仓评价」留在明细里只会让人
   * 以为系统还在算这笔账。总分从流水重建（Σevent = 总分），所以丢掉即等于退分。
   *
   * 直接交易那三项里比较特殊：分不扣了，但**笔数要留**——决策占比的分母靠它，
   * 而且它是统计不是惩罚。所以这里仍按「当日该标的一笔」去重计数，只是不再累减分。
   */
  const repriced = [...state.events].reverse().map(event => {
    if (RETIRED_EVENT_TYPES.has(event.type as string)) {
      if ((event.type as string) === 'direct_reward') {
        const key = `${event.date}|${event.trade?.symbol ?? '__nosym__'}`;
        if (!seenDirect.has(key)) {
          seenDirect.add(key);
          directTradeCount += 1;   // 笔数保留，分不再扣
        }
      }
      return null;
    }
  ...
  }).filter(Boolean).reverse() as ExecutionAssetEvent[]; // 去掉被删事件,复原「新在前」

  return {
    ...state,
    points,
    directTradeCount,
    // 三项已删除，累计计数归零，否则卡片会显示一个再也不会变的历史残值。
    campaignMissingCount: 0,
    events: repriced,
    scoringVersion: EXECUTION_SCORING_VERSION,
  };
