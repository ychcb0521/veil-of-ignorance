...
function buildLegQuestionAnswers(leg: TradeJournal): QuestionAnswer[] {
  ...
  add(answers, '这笔交易的结果是什么？', leg.post_outcome, OUTCOME_LABEL);
  add(answers, '这笔交易的已实现盈亏是多少？', leg.post_realized_pnl);
  add(answers, '这笔交易最终实现了多少 R？', leg.post_r_multiple);
  add(answers, '结果复盘总结是什么？', leg.post_result_summary);
  add(answers, '这笔交易的决策质量如何？', leg.post_decision_quality, DECISION_QUALITY_LABELS);
  add(answers, '这个对冲值回成本了吗？', leg.hedge_worth_it, HEDGE_WORTH_IT_LABELS);
  ...
  add(answers, '预设的亏损原因兑现没有？', leg.post_premortem_review);
  add(
    answers,
  ...
  add(answers, '证伪状态的补充说明是什么？', leg.exit_falsification_note);
  add(answers, '真实的离场 / 证伪事实是什么？', leg.post_invalidation_review);
  add(
    answers,
    '建仓时的盈亏比目标在实际走势中表现如何？',
    oddsReview.review,
  ...
  add(answers, '盈亏比目标表现的事实依据是什么？', oddsReview.body);
  ...
  add(answers, '建仓时盈亏比估计的复盘说明是什么？', leg.post_entry_payoff_basis_review);
  add(
    answers,
    '建仓时胜率估计属于哪一档？',
  ...
  add(answers, '建仓时胜率估计的复盘说明是什么？', leg.post_entry_win_rate_basis_review);
  add(answers, '机会质量评估中的预期盈亏比 b 是多少？', leg.post_opportunity_quality_payoff_ratio);
  add(answers, '机会质量评估中的预期回撤 d% 是多少？', leg.post_opportunity_quality_drawdown_pct);

  add(answers, '这笔交易选择了哪一种路径？', leg.post_path_mode, PATH_MODE_LABELS);
  add(answers, '这笔交易的主动权评分是多少？', leg.post_trade_agency_score, TRADE_AGENCY_LABELS);
  add(
    answers,
    '这笔交易过程中的纠结度 / 轻松度如何？',
  ...
  add(answers, '反对者的判断后来被证明是对的吗？', leg.post_opponent_was_right);

  for (const question of CLOSE_REVIEW_AUDIT_QUESTIONS) {
    add(answers, question.question, reflection.answers[question.key]);
  }
  add(answers, '平仓复盘还有哪些补充？', reflection.legacyText);
  add(answers, '下一次最应该坚持或修正的动作是什么？', leg.post_correct_action);

  add(answers, '情绪七问 ① 这单最起波澜的事情是什么？', leg.post_emo_disturbance);
  add(answers, '情绪七问 ② 我的第一反应是什么？', leg.post_emo_first_reaction);
  add(answers, '情绪七问 ③ 我其实想得到什么？', leg.post_emo_wanted);
  add(answers, '情绪七问 ④ 我其实在害怕什么？', leg.post_emo_feared);
  add(answers, '情绪七问 ⑤ 我给自己找了什么理由？', leg.post_emo_excuse);
  add(answers, '情绪七问 ⑥ 这单捞起的主石头是什么？', leg.post_emo_main_stone);
  if (leg.post_emo_main_stone_tags?.length) {
    const labels = leg.post_emo_main_stone_tags.map(tag => (
      MAIN_STONE_META[tag as keyof typeof MAIN_STONE_META]?.label ?? tag
    ));
    add(answers, '情绪七问 ⑥ 主石头标签有哪些？', labels);
  }
  add(answers, '情绪七问 ⑦ 明天再遇到同样的事情，我准备怎么选？', leg.post_emo_next_time_plan);

  add(answers, '五步诊断：我的目标是什么？', leg.post_five_step_goal);
  add(answers, '五步诊断：阻碍目标实现的问题是什么？', leg.post_five_step_problem);
  add(answers, '五步诊断：近因是什么？', leg.post_proximate_cause);
  add(answers, '五步诊断：根因是什么？', leg.post_root_cause);
  add(answers, '五步诊断：修正方案是什么？', leg.post_design_intervention);
  add(answers, '五步诊断：修正方案属于哪一类？', leg.post_intervention_type, INTERVENTION_TYPE_LABELS);
  add(answers, '五步诊断：如何监督执行？', leg.post_execution_monitor);
  add(answers, '五步诊断：最薄弱的环节是什么？', leg.post_five_step_weak_point, FIVE_STEP_WEAK_POINT_LABELS);

  add(answers, '错误场景是什么？', leg.post_error_scenario);
  add(answers, '原始假设是什么？', leg.post_original_hypothesis);
  add(answers, '现实反馈是什么？', leg.post_reality_feedback);
  add(answers, '错误类型总结是什么？', leg.post_error_type_summary);
  add(answers, '真正的问题是什么？', leg.post_real_problem);
  add(answers, '修正后的规则是什么？', leg.post_new_rule_draft);
