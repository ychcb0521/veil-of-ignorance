          ...
          <section id="s4" className="scroll-mt-20">
            ...
            <section id="s4-3" className="scroll-mt-20">
              ...
              <ul className="list-disc pl-6 text-[14px] text-foreground/90 space-y-1">
                ...
                <li><strong>「Δb」列与主力阶段拆解。</strong>Δb = 该腿盈亏 ÷ 战役初始最大预期亏损 L——这条腿把整场盈亏比 b <strong>推高或拉低了多少个单位</strong>，Σ(各腿 Δb) 恰为战役已实现 b，「b 是如何被增加、如何被削减」由此逐腿可见。更进一步，<strong>主力不是铁板一块：每一次滚动对冲的结束，意味着主力一个阶段的完成</strong>。主力行下方会展开浅色的<strong>阶段子行</strong>：按各次对冲的平仓时刻把主力切段（边界价即对冲平仓价——同一时刻的市价），每段给出时间区间、起止价、分摊盈亏、贡献率与 Δb。分摊按价差权重进行、<strong>各段之和严格等于主力整腿盈亏</strong>（Σ阶段贡献 = 主力贡献、Σ阶段 Δb = 主力 Δb，三重守恒），绝不臆造数字。一场「整腿 +3.2 万」的主力可能拆出「对冲结束前 +4.0 万、之后回吐 −0.8 万」——哪一段决策在挣钱、哪一段在回吐，据此可见。贴着主力平仓结束的对冲不切段；只有一段时不显示子行。PNG 导出同源同列。</li>
                ...
                <li><strong>Legs 列表每条腿都标明开仓/平仓时间、开仓价/平仓价、仓位与状态</strong>；还没平仓的腿，平仓时间与平仓价显示「—」。平仓价右侧的<strong>「涨跌幅」</strong>按持仓方向计（多单 =（平仓价 − 开仓价）÷ 开仓价，空单 =（开仓价 − 平仓价）÷ 开仓价），正数即这条腿在价格上占优、按所示的这一对开平价看与盈亏同号；与左边两格用同一对价（含 K 线平仓价校正），未平仓显示「—」；一个仓位分几刀平掉时平仓价取最后一刀、盈亏是各刀合计，两者符号可能不同；主力阶段子行按主力方向、各自起止价各算各的，合计行留空。PNG 导出同列。「币量 / 仓位」右侧的<strong>「占比」</strong><strong>多单、空单分开算、各自 100%</strong>：按这条腿实际的持仓方向分组（与「涨跌幅」同一个方向），多单各腿只和多单比、空单各腿只和空单比——对冲通常是空单，主空战役里的对冲则是多单，分组跟方向走、不跟角色走；每格上行的数字前挂一枚<strong>「多」（绿）/「空」（红）标签</strong>，一眼分清是哪一组，百分数本身仍是中性色，悬停可见「多单合计里的占比」/「空单合计里的占比」。上行是这条腿的币量占同方向币量合计的百分比、下行是名义仓位占同方向名义仓位合计的百分比，同一方向里两个分母也各算各的，用的是「币量 / 仓位」列每行背后的同一组数，按未舍入的原值相加（各行与合计行各自取两位小数，把各行印出来的数手工相加，末位可能与合计行对不上）；状态为「挂单中」的对冲 / 镜像腿（还没有成交或平仓记录）不计入合计、两行都显示「—」、不挂标签，缺开仓价的腿上行显示「—」、不进本方向的币量合计；各行分别取一位小数，同一方向逐行相加不一定恰好是 100.0%（腿越多、舍入差可能越大，如六条等额的多单各印 16.7%，加起来是 100.2%）；阶段子行留空。合计行分别给出多、空两个分母：「币量 / 仓位」格每个方向写一组（以同样的标签开头，上行 Σ币量、下行 Σ名义仓位），「占比」格与之逐组对齐写「多 100.0%」/「空 100.0%」，没有计入腿的方向不列（例如唯一的对冲还挂单中时只剩多单一组）；一条腿都不计入时这两格都显示「—」（某方向计入的腿都缺开仓价时，该方向只有上行是「—」）。PNG 导出同列、同样的彩色标签。「委托」列按真实业务归属呈现：委托先按挂出时刻归到当时开着的那笔主力；<strong>加仓之后挂出的委托放在加仓那一行的后面，再加仓就放到最新那次加仓的行后面</strong>（按开仓时刻排，不看「加仓1 / 加仓2」的编号；加仓开出前 5 分钟内预挂、且加仓开出时仍挂着的也算这次加仓；主力开仓前预挂的仍归主力；加仓先平掉之后挂出的回到主力；反过来主力先平、加仓还开着时仍接在加仓后面；全部平完之后才挂出的，与收尾前一刻挂出的落在同一行）。这只是 Legs 表与导出 PNG 的<strong>展示位置</strong>，初始暴露、预期最大亏损等风险指标仍以主力为锚。对冲触发后的反向委托放在对应的对冲 leg；镜像止盈行则显示止盈挂单时间与触发时间，避免把主力委托误记到镜像腿。</li>
                <li><strong>「加仓校验」列</strong>（紧跟「币量 / 仓位」与「占比」之后）只在加仓行打标：每笔加仓只按<strong>它自己那一刻</strong>的客观状态判定（马尔可夫式）：当时仍持有的全部腿按开仓价入账——更早的加仓在当时止损线上的浮盈加进垫子、浮亏扣减垫子——加上当时挂着的止损线与当时已落袋的 G；不继承上一笔加仓的对错，上一笔判 ✗ 不会让这一笔自动判 ✗，上一笔的浮亏尚未兑现、而当下已出现可加仓的优势时，照当下判。以加仓那一刻挂着（或加仓后 5 分钟内补挂）、在亏损侧离加仓价最近的反向委托价为止损线 S₁（加仓计算器比对盘口线用同一规则），<strong>旧仓浮盈垫 X₁(S₁ − S̄) + 已落袋 G ≥ 新加仓最大预期亏损 X₂(S₂ − S₁)</strong>（主空符号翻转，不计手续费；币本位 G 按 S₁ 折成 U）即合规——X₁ 只算加仓那一刻还拿着的币（加仓前已减掉、已止盈的不算）；G 的正向来源只认成交记录上退出方式为止盈1 的那几刀（镜像止盈），普通减仓、手动或止损平仓的盈利不混入——哪怕合并仓位按成交占比分到了镜像腿上；先前已经实现的亏损（含强平）则从 G 中扣掉，G 可以是负数；再入场之前那一轮也不算。与加仓计算器同一个 G。合规时显示一枚几乎隐形的淡灰对号；抹不平就是<strong>红色放大的叉</strong>，下方直接写出 Plan B 的<strong>正确加仓上限</strong>：X₂ 是币量，同时给出按 S₂ 折算的 U 名义仓位，两个数是同一仓位的两种单位，不可相加。<strong>点击红叉</strong>会展开计算框，逐步列出 Y₁、G、可用覆盖额、每币风险、正确币量上限、U 名义仓位以及实际超出量；读不到 S₁ 等无法判断时显示淡灰「—」。PNG 导出也会写明币量上限与 U 折算额。<strong>S₂ 一律按成交价判</strong>——市价单含引擎滑点（0.01% + 名义 ÷ 50 亿）的那个价，不是下单前看到的现价；真正的风险就在成交价上，判定不为计算器让步。成交记录带着加仓计算器当时的计划时，计算框与 PNG 会并排写出「计算时 现价 …，预计成交 …，上限 … 币；实际成交 …，上限 … 币」（括号里是相对这张单下单价的滑点；「现价」只用于市价计划——限价计划的参考价是手填的限价，写「计算时 限价 …，挂单价 …」，条件委托计划写「计算时 触发价 …」），再说清超出从哪来：
                ...
                <li><strong>镜像止盈即使没有触发，也会作为一条 leg 列出</strong>（状态显示「挂单中」）。未触发意味着没有成交记录、因而没有对应的成交行，但「这场战役到底挂没挂过镜像止盈、挂在什么价位」本身就是复盘要看的事实——不能因为它没成交就在列表里消失。</li>
