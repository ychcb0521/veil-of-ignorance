...
P 不再一把手填，而是拆成三个更可回答的问题：<strong>「这个结构还活得下去吗（P₁）」</strong>、<strong>「假如它活着，价格能走到 T 吗（P₂）」</strong>、<strong>「假如它死了，价格还是走到了 T 吗（P₃）」</strong>。P₂ 与 P₃ 都必须按<strong>条件概率</strong>来估——先假定结构的死活，再问突破的把握，否则会把结构风险重复计价。
            ...
            <section id="s3-1c" className="scroll-mt-20">
              ...
              <P>
                <strong>加仓量：镜像尚未落袋时看 Plan A；镜像已经落袋后，统一按 Plan B 下单。</strong>Plan A 只计算当前仍持有旧仓退回新止损线时的净浮盈垫；Plan B 再把本轮已经落袋的净利润并入。每次加仓都用<strong>当下仍持有仓位的 X₁ 与最新综合成本 S̄</strong>重算，因此前一笔仍持有的加仓不会消失：它在新 S₁ 上的浮盈或浮亏会自动进入本轮垫子。三个价格定义一个区间：<strong>S̄ 当前综合成本 · S₁ 新止损线 · S₂ 本次加仓价</strong>。
              </P>
              <div className="my-3 rounded bg-muted/60 px-3 py-2 font-mono text-[11px] leading-relaxed text-foreground">
                Plan A：Y₁ = X₁ (S₁ − S̄)；X₂ᴬ = Y₁ ÷ |S₂ − S₁| = X₁ ÷ b<br />
                Plan B：X<sub>add,max</sub> = (Y₁ + G) ÷ |S₂ − S₁| = X₂ᴬ + X<sub>G</sub>；Y₁ 可为负<br />
              ...
              <P>
                <strong>顶栏「加仓」按钮就是这套公式的计算器</strong>（在「倒叙播放」左侧）。X₁ / S̄ 按当前仍持有各腿的开仓价折算后读入、S₂ 读现价，三者都可手改并一键复位；S₁ 必须手填——那是你的判断。界面把来源拆成两块，但不是让你二选一：<strong>Plan A · 旧仓浮盈垫</strong>显示 Y₁ 与其折算量 X₂ᴬ，只用于解释旧仓贡献；镜像利润一旦落袋，真正下单必须看<strong>Plan B · 浮盈垫 + 落袋镜像</strong>的总上限 <span className="font-mono">(Y₁ + G) ÷ 险</span>。系统会把 Y₁ 的正数加进来、负数扣出去，绝不能绕过总上限单独照 X<sub>G</sub> 下单。K<sub>B</sub> 是可选的落袋垫旋钮，留空即取 S₁；此时 <span className="font-mono">X<sub>G</sub> = G ÷ |S₂ − S₁|</span>，跌回 S₁ 恰好花掉 G。对冲必须扛起全部实际仓位：<span className="font-mono">对冲 @ S₁ = X₁ + Plan B 加仓量</span>。把 K<sub>B</sub> 拖到 S₁ 的更保守一侧，表示只在更远的 K<sub>B</sub> 才花完 G；界面会同步重算这部分加仓与在 S₁ 的剩余垫子。
              </P>
              <RedHighlight>
                ...
                <br /><br />
                <strong>另一个同源的坑</strong>：镜像已落袋后，A 段不再是最终下单答案。
                真正要下的是 <span className="font-mono">Plan B 加仓上限 = (Y₁ + G) ÷ 险</span>、
                真正要挂的是 <span className="font-mono">合计对冲 = X₁ + Plan B 加仓量</span>——
          ...
          <section id="s4" className="scroll-mt-20">
            ...
            <section id="s4-3" className="scroll-mt-20">
              ...
              <ul className="list-disc pl-6 text-[14px] text-foreground/90 space-y-1">
                ...
                <li><strong>「加仓校验」列</strong>（紧跟「币量 / 仓位」）只在加仓行打标：以加仓那一刻挂着（或加仓后 5 分钟内补挂）、在亏损侧离加仓价最近的反向委托价为止损线 S₁，<strong>旧仓浮盈垫 X₁(S₁ − S̄) + 已落袋 G ≥ 新加仓最大预期亏损 X₂(S₂ − S₁)</strong>（主空符号翻转，不计手续费）即合规——X₁ 只算加仓那一刻还拿着的币（加仓前已减掉、已止盈的不算）；G 的正向来源只认本轮镜像止盈 / tp1，普通减仓或手动平仓的盈利不混入，先前已经实现的亏损则从 G 中扣掉，再入场之前那一轮也不算。合规时显示一枚几乎隐形的淡灰对号；抹不平就是<strong>红色放大的叉</strong>，下方直接写出 Plan B 的<strong>正确加仓上限</strong>：X₂ 是币量，同时给出按 S₂ 折算的 U 名义仓位，两个数是同一仓位的两种单位，不可相加。<strong>点击红叉</strong>会展开计算框，逐步列出 Y₁、G、可用覆盖额、每币风险、正确币量上限、U 名义仓位以及实际超出量；读不到 S₁ 等无法判断时显示淡灰「—」。PNG 导出也会写明币量上限与 U 折算额。</li>
