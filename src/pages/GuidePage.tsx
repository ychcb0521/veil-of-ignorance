        ...
        <main ref={mainRef} className="guide-doc min-w-0">
          ...
          <section id="s3" className="scroll-mt-20">
            ...
            <section id="s3-0" className="scroll-mt-20">
              ...
              <P>
                交易页<strong>顶部 Header</strong>（标的选择器与右侧「复盘中心」之间）有一对开关：<strong>决策记录</strong> 与 <strong>直接交易</strong>。这是进入交易页后你做的第一个决定，也是整套系统里最大的一个分叉——它决定本次会话产生的数据是否进入复盘体系。系统默认 <strong>直接交易</strong>，需要训练时手动切换到决策记录。紧挨在「直接交易」右边、隔一条细线的是另一对开关「<strong>无限制 / 币安标准</strong>」——<strong>持仓限制模式</strong>，决定杠杆、持仓与单笔下单有没有上限（见本节末尾）。再往右那个极小、近乎隐形的符号，是「<strong>同步 / 隔离</strong>」<strong>时间模式</strong>开关（点开才展开，详见 3.2）——那是切换币种时的时间推进方式，别和这里的两组模式混为一谈。
              </P>
              ...
              <P className="mt-6">
                <strong>持仓限制模式：无限制 / 币安标准。</strong>开关在顶部 Header「直接交易」的<strong>右边</strong>（隔一条细线），两段：<strong>无限制</strong> 与 <strong>币安标准</strong>。系统默认 <strong>无限制</strong>——从没选过的老用户也是无限制；选了哪一个按账号保存，换浏览器也跟着走。随时可以切，无确认弹窗；鼠标停在两段上，按钮正下方会显示这一档的说明；切换时消息中心记一条——切到币安标准时，若挂着的委托里有按币安规则到触发 / 成交时会被撤的（尤其是按成数挂的止盈止损），这一条升为警告并写出张数。
              </P>
              <div className="mt-4 grid gap-4 md:grid-cols-2">
                <div className="flex h-full flex-col rounded-xl border border-border/70 bg-card/95 p-5 shadow-sm">
                  <div className="border-b border-border/50 pb-3">
                    <div className="text-[13px] font-semibold tracking-[0.01em] text-foreground">无限制（默认）</div>
                  </div>
                  <div className="pt-3 text-[13px] leading-7 text-muted-foreground">
                    <strong>无任何杠杆限制、无任何仓位大小限制</strong>：任何币种（U 本位、币本位、币安没有的合成币本位都一样）杠杆 <strong>1–150x</strong>（150x 是币安全部合约里最高的那一档，所以没有哪个币种比币安更严）；不判「持仓 + 当前委托 + 本单」的持仓上限；没有单笔市价 / 限价上限；有持仓时<strong>也能降杠杆</strong>（降杠杆要从可用余额追加保证金，补不上才拒）。下单面板不显示杠杆分层与单笔上限小字，当前委托里也不标「触发时将超限」「触发时将超单笔上限」，「可开」只受可用余额 × 杠杆约束。一单大到滑点把成交价推过它自己的强平价时（150x 按 0.4% 维持保证金，强平价离开仓价只有约 0.27%，名义约 1,280 万 USDT 以上的市价单就会这样），下单面板在按钮前标红「一成交就会被强平」——只提醒，不拦。在这个模式下开出的仓位按<strong>统一 0.4%</strong> 计维持保证金（与更新前的仓位同一个公式）——大名义、高杠杆的仓位不会因为分层维持保证金一开出来就被强平。保证金是否够、强平、手续费、滑点照常。
                  </div>
                </div>
                <div className="flex h-full flex-col rounded-xl border border-border/70 bg-card/95 p-5 shadow-sm">
                  <div className="border-b border-border/50 pb-3">
...
P 不再一把手填，而是拆成三个更可回答的问题：<strong>「这个结构还活得下去吗（P₁）」</strong>、<strong>「假如它活着，价格能走到 T 吗（P₂）」</strong>、<strong>「假如它死了，价格还是走到了 T 吗（P₃）」</strong>。P₂ 与 P₃ 都必须按<strong>条件概率</strong>来估——先假定结构的死活，再问突破的把握，否则会把结构风险重复计价。
            ...
            <section id="s3-1c" className="scroll-mt-20">
              ...
              <RedHighlight>
                ...
                <strong>可下单量还要过币安分层</strong>（见「杠杆分层与仓位上限」；只在<strong>币安标准</strong>持仓限制模式下——默认的无限制模式没有分层上限与单笔上限，只受 Plan B 约束，见 3.1）：按这个合约当前的杠杆，这一侧还能再开多少——持仓多空相加、非只减仓挂单都算，
          ...
          <section id="s4" className="scroll-mt-20">
            ...
            <section id="s4-3" className="scroll-mt-20">
              ...
              <P>战役卡片默认只保留两层核心信息：第一层是标题、方向、标的、杠杆倍数（仓位击穿的战役紧跟一枚红色的「仓位击穿」标签）、唯一编号、<strong>真实操作时间</strong>、重要性与结束状态；第二层是八项指标，<strong>顺序与排序行一致</strong>：镜像止盈状态、预期回撤、涨跌幅、涨跌幅倍数、盈亏比、加仓效用、单场几何期望、单场算术期望。第二层<strong>左对齐、紧凑排开</strong>：每项上方一行淡色的指标名、下方一行等宽数字，从左往右依次排列，右侧留白、不把整行均分；每项的宽度按<strong>当前列表里实际出现的读数</strong>定：取这一项的指标名与当前时间段里全部战役最宽的那个读数，再加左右内边距——读数都短的列表不再为没出现的极端读数留空，相邻两项之间的空白因此匀称；换一个时间段，宽度跟着这一段的战役重算；<strong>切换排序不改宽度</strong>（有的排序会筛掉算不出这一项的战役，宽度仍按整个时间段算，文字不会左右挪）；首次加载时战役分批到达，宽度可能随新到的读数放宽，加载完就定下来。所有卡片用同一套宽度，<strong>同名的项在上下各张卡片上落在同一条竖线上</strong>。手机上放不下一行，改成每行两项。<strong>盈亏比只写倍数 b</strong>（如「34.60」「-0.80」），不再写百分数和括号。<strong>当前排序项在封面上高亮</strong>：按哪一项排序，每张卡片上对应的那一项就加一层淡琥珀底与细描边、指标名变成琥珀色；按操作时间、杠杆倍数、重要性排序时亮的是标题行里的操作时间、杠杆徽标与重要性，按字母排序时标题下面多一道琥珀下划线。高亮只换底色，切换排序时文字不会挪动。「机会质量」已删掉：它用 max（b, 1）÷ 预期回撤把亏损场一律抹成同一个数，涨跌幅倍数（主力涨跌幅 ÷ 预期回撤）衡量同一件事更直接。指标为正时使用绿色、为负时使用红色，无法计算时显示「—」。</P>
              ...
              <P><strong>多级排序</strong>：<strong>单击</strong>排序项仍是只按这一项排（已经只按它排时再单击切换方向）。要在它的基础上再排一层，把鼠标移到另一项上，点它右上角出现的<strong>「+」</strong>，这一项就追加为下一级——例如按镜像止盈排着，再点加仓效用的「+」，镜像止盈同一档里的战役就按加仓效用从大到小排；还可以接着加第三级、第四级。手机上没有悬停，<strong>长按排序项</strong>加一级（短按仍是只按它排）。多级时排序行下方出现一条<strong>排序链</strong>，如「① 镜像止盈 ↓ › ② 加仓效用 ↓」：点某一级的名称或箭头单独切换它的方向，点 × 单独移除这一级（移除第一级时第二级升为第一级），<strong>「清除」回到单级、保留第一级</strong>（连同它的方向）；只有一级时这条链不出现，界面与原来一样。「+」挂在排序项右上角外沿、只盖住角上一小块，不挡文字与 Σ，排序项的单击、双击与右键都和原来一样；多级时单击链上的某一项＝只按它排、方向不变，双击看说明不改排序链。排序行里链上各项的右上角（「+」原来的位置）标出第几级；封面上链里每一级对应的指标都高亮——第一级是原来的琥珀底与描边，之后各级轻一档。<strong>进不进列表只由第一级决定</strong>（与只按它排时的口径完全一致，空列表的提示也按第一级说）；第一级打平时按第二级比较，再打平看第三级……各级都打平后，退回第一级原有的并列规则。<strong>第二级起算不出的战役留在本档、排到本档末尾</strong>（不论这一级是升序还是降序），封面照常显示「—」。排序链写进地址栏（第一级仍是 sort / direction，之后每级一个 then=项.方向），从详情页返回时原样回来；批量下载的「按当前排序选择」按整条链取前 N 场；「+」加层与链上的改动不会切换已打开的散点图，单击排序项才会。<strong>第一级是连续数值指标时分档</strong>：第一级是预期回撤、涨跌幅、涨跌幅倍数、盈亏比、加仓效用、几何 / 算术期望这类连续数值指标、且链上不止一级时，它几乎没有并列，第二级本来无从起作用——于是先按四分位把进入列表的战役分成四档（档界按当前列表算，降序时 Q4 在前），同档内按后面各级排，各级都打平再按第一级本身的数值、最后才是它原有的并列规则；排序链上第一级标着<strong>「分档」</strong>，悬停或点它（也可点 ⓘ）看档界（如「Q4 ≥ 3.21 · Q3 ≥ 1.05 · …」）——档界按封面精度取整、就是那一档里最小的读数，封面读数相同的战役必在同一档；<strong>有意义的分界线一定是档界</strong>（盈亏比 0 与 −1R、加仓效用 1 与 0、涨跌幅 / 涨跌幅倍数 / 算术期望 0、几何期望 0.90 与 1.00）：四分位切出的某一档若跨过其中一条，就把它靠那条线的档界挪到线上（这一档线下读数多就只留线下、否则只留线上），负值永远不与正值同档，恰好落在线上的归上面一档，这条档界显示为分界线本身（如 0.00、-1.00、1.00、0.90）；四档装不下所有分界线时，按上面列出的先后优先；封面第一级的高亮不变。镜像止盈 / 重要性 / 杠杆倍数 / 字母 / 操作时间不分档，只有一级时也不分档（与原来逐位相同）。第二级起每一级还标着<strong>「本级排了 N 场」</strong>——前面各级并列的战役里按这一项分出先后的几场，算不出的几场留在组尾（悬停或点它看明细）；<strong>「未起作用」</strong>= 前面各级没有并列、并列的读数全相同（比如某一档里全是 5 星），或并列的都算不出这一项——加仓效用只有做过加仓、且涨跌幅倍数为正的战役才算得出，其余在各档末尾显示「—」，这不是排序失灵。ⓘ 末尾的「当前」段逐级写出当前的档界与各级的作用，手机与键盘同样读得到。</P>
              <P><strong>排序后的分组统计</strong>：在排序链上<strong>双击或右键</strong>任一级，ⓘ 浮层顶部会列出一张统计表——按第一级把列表切成组（连续指标一档一行、镜像止盈 / 重要性 / 杠杆一个读数一行，字母与操作时间只给「全部」一行），每组报<strong>场数、胜率（b &gt; 0 的场数 ÷ 算得出 b 的场数）、平均 b</strong>，后面各级报本组的读数概况：连续指标与杠杆写平均值，镜像止盈写生效场数，重要性写平均星级。组的先后与列表相同，末行是合计。统计表下面，后面每一级各有一张<strong>交叉表</strong>：行是第一级的档，列是这一级自己的档（连续指标按整张列表统一分四分位，档界是那一档里最小的读数，同样以上述分界线为档界；镜像止盈 / 重要性 / 杠杆一个读数一列；算不出的另起一列），格子是场数、下面小字是占本行的比例，底色越深占比越大——直接读出「第一级的每一档里，第二级集中在哪一段、各有几场」。表格最下方有一行小字脚注说明 Q1–Q4 的含义（Q1 最低四分之一、Q4 最高四分之一，「≥ x」是档内最小读数）。双击不会改动排序方向。</P>
              <P>默认按<strong>真实操作时间</strong>从新到旧排序。排序行依次是操作时间、镜像止盈，一条短分隔线之后是<strong>预期回撤、涨跌幅、涨跌幅倍数、盈亏比、加仓效用、几何期望、算术期望</strong>七项排在一起（与封面上镜像止盈之后的七格同序），再一条短分隔线之后是杠杆倍数、重要性与字母（重要性放在后面，排在字母之前）；排序行原有的「DSI 贡献」「USI 贡献」两项已删掉——它们与盈亏比几乎同序，单场的 DSI/USI 贡献仍在详情页「盈亏概览」里，整表的 DSI / USI 仍在统计概览的「不对称风险」里；每一项都可双向排序。排序行<strong>左对齐</strong>、按钮依次排开、间距均匀，放不下时整行换行。选中的那一项在 Σ 的位置换成方向箭头；没有公式的操作时间、杠杆倍数、字母平时也留着同样宽的空位，切换排序时按钮不会换位。涨跌幅、涨跌幅倍数、加仓效用与其他公式指标一样，<strong>双击或右键</strong>打开公式浮层：公式、每个符号的含义、一个代入数字的例子、哪些战役不参与，底部是「查看散点图」入口。<strong>涨跌幅</strong>按战役算，按主力方向计，空单价格跌了为正：<strong>开仓价取主力各笔里最有利的那个</strong>（主多最低、主空最高）；<strong>平仓价看主力平仓那一刻有没有对冲把行情锁住</strong>——滚动对冲（<strong>已触发的初始对冲 A/B 在这里也算滚动对冲</strong>）仍持有、或与主力同一次操作里平掉（相差不超过一分钟），都算锁住；<strong>回场对冲只认与主力同一次操作里平掉</strong>（主力平仓后它仍持有不算）。有锁住的对冲，就取其中最早开的那张的开仓价——对冲一成交，主力后面的行情就被锁住了；没有就取主力自己的平仓价（最后平的那几笔里最有利的）。触发后又撤掉的对冲，撤单那一刻就不在手了。盈亏概览「涨跌幅」的 ⓘ 里写明本场取的两个价和来自哪张腿；开平价与详情页 Legs 表同源（含平仓价校正）。涨跌幅倍数、加仓效用、散点图、盈亏概览与反事实都从这一个数派生；主力都还没平仓的战役不进入这一档排序，卡片第二行常驻显示这个数（紧跟「预期回撤」）。<strong>涨跌幅倍数 = 主力涨跌幅 ÷ 预期回撤</strong>（两者都是价格层面的百分数，结果是倍数）：主力涨了 12%、入场到对冲边界 4%，倍数就是 +3.00——价格走出了 3 个「预期回撤」。主力未平仓或算不出预期回撤的战役不进入这一档排序；卡片上紧跟「涨跌幅」显示，悬停给出代入值。<strong>加仓效用 = 盈亏比 ÷ 涨跌幅倍数</strong>：只拿主力、不加仓时，盈亏比大致就是主力的涨跌幅倍数（最大预期亏损按入场到对冲边界的距离定），比值约为 1；大于 1 说明加仓把同一段行情放大成了更多的 R，小于 1 说明加仓、对冲或止盈吃掉了行情。<strong>只在做过加仓、且涨跌幅倍数为正时计算</strong>（有一条成交过的加仓腿；没有加仓时这个比值恒在 1 附近，没有信息量；涨跌幅倍数为负时亏损战役负负得正会排到最前，接近 0 时分母过小会把主力几乎没动的战役放大成十几倍，所以按显示到两位小数的值判，0.00 也不算），其余战役不进入这一档排序与散点图；卡片上紧跟「盈亏比」显示，算不出时显示「—」。这里的操作时间是客观发生时间，不是无知之幕时间机器里的模拟时间。</P>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-[11px] my-3 border border-border rounded overflow-hidden">
                  ...
                  <tbody>
                    ...
                    <tr><td className="px-3 py-2 border-t border-border">单场盈亏比 bᵢ</td><td className="px-3 py-2 border-t border-border">已实现盈亏ᵢ ÷ Lᵢ</td><td className="px-3 py-2 border-t border-border">盈利为正，亏损为负；页面只写倍数 bᵢ（两位小数，如 34.60、-0.80），不写百分数；不到 0.005 的读作 0.00、用中性色，不按原始正负上红绿</td></tr>
              ...
              <P>盈亏比、预期回撤、涨跌幅、涨跌幅倍数、加仓效用、算术期望、几何期望、重要性、镜像止盈都各自配有一张散点图。在排序行上<strong>双击或右键</strong>对应指标打开公式浮层，浮层底部的「查看散点图」即可展开；再点一次「收起散点图」关闭。<strong>盈亏比、涨跌幅、涨跌幅倍数、加仓效用、算术期望与几何期望默认展开的是分布图、镜像止盈与预期回撤默认展开的是柱状图</strong>，其余指标以及这几项切回「时序」后，横轴都是按<strong>客观操作时间</strong>排列的战役序号，与列表当前排序无关——改排序不会让点位移动。</P>
              <ul className="list-disc pl-6 text-[14px] text-foreground/90 space-y-1">
                <li><strong>颜色分两类。</strong>本身带盈亏方向的指标（盈亏比、涨跌幅、涨跌幅倍数、加仓效用、算术期望、几何期望）按数值正负着色：正绿、负红、零灰。不带方向的纯量级指标用单色，避免暗示盈亏：重要性琥珀。</li>
                <li><strong>涨跌幅、涨跌幅倍数、加仓效用、算术期望默认看分布</strong>，与盈亏比同一套画法：横轴就是指标本身（涨跌幅 %、涨跌幅倍数与加仓效用「倍」、算术期望 R，线性刻度），纵轴是落在该值附近的战役数量——点按档从底线向上堆叠，灰色曲线是密度估计，不考虑时间先后。灰色 0 线分开正负：涨跌幅与涨跌幅倍数的 0 叫「不涨不跌」，摘要条报「顺向」占比（价格朝主力方向走的场数）；算术期望的 0R 是盈亏平衡（对应 b = +1R），摘要条报「正期望」占比；加仓效用另有一条<strong>琥珀色 1.00 虚线</strong>「加仓没有额外放大」，摘要条多报一项「放大（&gt; 1）」占比。0 与 1.00 都是档边界，点不会吸附到线的另一侧，恰好落在线上的归右侧；窗口取 p2–p98 并一定把 0（和 1.00）圈进来，超出的极端值贴边画成三角并计数。用面板右上角的「时序 | 分布」切回时序：纵轴是同一个数，横轴按客观操作时间排列。与排序同一组函数取数：主力未平仓的不进涨跌幅图，还算不出预期回撤的不进涨跌幅倍数图，<strong>没有加仓、或涨跌幅倍数不为正的战役不进加仓效用图</strong>，算不出盈亏比的不进算术期望图；未进图的场数写在图下脚注里。</li>
                ...
                <li><strong>预期回撤默认看柱状。</strong>柱子按预期回撤百分比的<strong>倒数 100 ÷ D%</strong> 等间距分档（D = 2% → 50）：越往右止损越紧，越往左回撤空间越宽。在 D 上等距分时，0.5%、1%、2% 这些紧止损会挤进同一根柱；换成倒数后它们被拉开。<strong>柱脚标的是实际的预期回撤区间</strong>（如「20–10%」「10–6.67%」，刻度左疏右密）与场数；样本够多且有极远的离群值时，最右一根并成「≤ 下界」柱。<strong>柱内自底向上是亏损 → 持平或进行中 → 盈利</strong>，换组时另起一行、一行里不混色，亏损越大越靠底、盈利越大越靠顶；颜色报该战役最终盈亏，悬停读预期回撤、倒数与 b。用面板右上角的「时序 | 柱状」切回时序，选择记进地址栏。</li>
              ...
              <SubTitle>交易战役：删除、回收站与平行世界</SubTitle>
              <P>删除战役采用可恢复模式：战役会从正常列表和统计中移出，但 Legs 与交易记录不会随之消失。列表标题栏右侧的低对比度回收图标可打开“已删除战役”，从中恢复原战役；只有在回收区再次选择“永久删除”时，战役归档才会真正移除。</P>
              <P><strong>进入回收站</strong>：「已删除战役」弹窗顶部有一个<strong>「进入回收站」</strong>按钮（回收站为空时停用），点它关掉弹窗、把列表切成<strong>回收站视图</strong>——回收站里的战役住在一个<strong>平行世界</strong>里：<strong>同一个页面、同一套功能</strong>，统计概览、操作时间段、排序（含多级排序与散点图 / 分布图）、封面指标行（列宽与排序高亮）、展开详情、重要性、状态标签、批量下载（选择模式与「前 N 场」）一项不少，<strong>只是数据源换成回收站里的战役</strong>。顶栏标题旁多一枚琥珀色的<strong>「回收站」</strong>标签，副标题写「只显示回收站里的战役」，回收图标点亮表示当前就在回收站；「归类历史交易」换成<strong>「返回战役」</strong>（新战役不会归类进回收站）。卡片右侧的删除按钮换成<strong>「恢复」</strong>与<strong>「彻底删除」</strong>（彻底删除要二次确认，与弹窗里的一致）：恢复或彻底删除后这张卡从回收站消失，恢复的战役下次回到正常列表就能看到。回收站里一场都没有时写「回收站是空的」；按当前排序口径筛空时的提示与正常列表相同。回收站视图写进地址栏（<strong>?bin=1</strong>，与 sort / direction / then / from / to / chart 并存），刷新、分享链接都保留；从回收站点进战役详情，详情页顶部有一条「这场战役在回收站里」的提示配「恢复」按钮，返回列表仍回到回收站视图。「已删除战役」弹窗照旧（恢复 / 彻底删除不变），只是多了这个入口；回收图标上的场数照旧。正常列表的读取、核对与后台自愈一字不变；回收站那一份缓存按「用户 + 作用域」单独建，从不做落库自愈（已删的战役不进统计）。</P>
              ...
              <ul className="list-disc pl-6 text-[14px] text-foreground/90 space-y-1">
                ...
                <li><strong>K 线盘面默认 2.1 倍视窗、5 分钟线</strong>：首次打开（以及换一场战役）时显示 <strong>2.1 倍</strong>战役时长的行情，战役持仓段居中，前后各留 0.55 倍上下文。底层同时预载战役左右各 25 倍的行情，完整可浏览范围为 51 倍；盘面顶部的小型倍率按钮可一键切换 <strong>1.1、2.1、3.1、5、11、21、31、41、51 倍</strong>，所有倍率都围绕战役持仓段居中，3.1 倍时左右各保留 1.05 倍上下文。<strong>周期默认 5 分钟线，放不下时自动放宽</strong>：当前视窗里 5 分钟线超过约 1200 根（例如 8 天的战役开 2.1 倍）、或 51 倍拉取超过 6000 根时，改用放得下的 15 分钟 / 1 小时线，不裁、不卡；周期按钮高亮的是盘面实际显示的周期，悬停可看这条规则。手动点过周期就按手动（绝对时间预设下仍不比可读下限更细）。历史战役同样适用。</li>
                <li><strong>周期只管盘面，读数不跟着变</strong>：峰值浮盈、盈亏概览、Legs 副本的基线与「一键运行」始终用按战役时长自动选周期的那一份 K 线计算（51 倍拉取窗口里 6000 根放得下的最细周期，与改版前默认打开时是同一份），切换盘面周期、倍数或「1天 / 1周 / 1月」都不会让这些读数变，时间预设下也能直接一键运行。盘面周期与这份计算周期相同时只拉一份 K 线、两边共用；不同时盘面另拉一份。计算用的那份加载失败而盘面正常时，盘面下方单独提示并可重试。唯一的例外是选中的反事实分支越出 Legs 的时间跨度：内容区间（见下一条）随之撑宽，这份 K 线的窗口与周期跟着变，读数也可能变（改版前就是这样）；取消选中即回到只按战役本身的那一份，与批量下载的盈亏概览一致。</li>
              ...
              <ul className="list-disc pl-6 text-[14px] text-foreground/90 space-y-1">
                ...
                <li><strong>盈亏概览分左右两栏，各自从上往下读</strong>：左栏是<strong>层层递进的一列</strong>——预期回撤、涨跌幅、涨跌幅倍数、盈亏比、加仓效用、几何期望、算术期望，与战役封面、列表排序栏同序，上一项是下一项的分母或来源（涨跌幅倍数 = 涨跌幅 ÷ 预期回撤，加仓效用 = 盈亏比 ÷ 涨跌幅倍数，两个期望都由盈亏比推出）；右栏是结果与仓位——第一个是<strong>最大预期亏损</strong>，与左栏第一个的预期回撤同一行（最大预期亏损 = 主力开仓名义仓位 × 预期回撤），之后是<strong>已实现 P&amp;L</strong>、紧跟着的<strong>峰值浮盈</strong>（USDT）、<strong>主力开仓名义仓位</strong>，第五是<strong>多方总名义仓位</strong>（主多战役里所有多单的名义仓位合计：主力、镜像、加仓都算，还挂着没成交的不算；就是 Legs 表合计行里多单那一组的 Σ 名义仓位；主空战役同理叫「空方总名义仓位」），之后是杠杆倍数、DSI/USI 贡献（只写这场进了哪一组、占组内 Σb² 的多少，如「USI 12.3%」；均方项 b²/n、样本数都在 ⓘ 说明里）。两栏各 7 项，左右同一行的两项始终齐平、每项只占一行；屏幕窄时并成一栏，先左栏、再右栏。卡片底部不再有「期望口径」脚注：算术期望的胜率统一取 50% 写在算术期望的 ⓘ 里，本场几何期望的资产分母用的是开仓快照还是今日总资产，写在几何期望的 ⓘ 里。</li>
              ...
              <SubTitle>交易战役：批量下载图片</SubTitle>
              <ul className="list-disc pl-6 text-[14px] text-foreground/90 space-y-1">
                <li><strong>进入选择模式</strong>：点列表「排序方式」一行最右端的<strong>批量下载</strong>（战役全部读完后才能点），排序行下方会展开一条选择条。勾卡片标题前的勾选框或直接点整张卡片，或者在任意散点图上点点位——时序、分布、柱状三种视图都可以点，两处共用同一份选择。选中的卡片描一圈琥珀色边、底色微微泛琥珀；散点图上已选的点加<strong>蓝色外圈</strong>（琥珀色外圈仍是破产风险提示，两者可以叠在同一个点上）。图高放不下、在顶端合成一个三角的那一档，点三角展开后逐场选择。往下翻卡片、选择条滚出视野（哪怕只被吸顶区盖住一点）时，屏幕底部会浮出一条「已选 N 场 · 退出选择 · 下载选中」，勾到哪里都能直接下载。手机等窄屏上选择条不吸顶（排序行本身已折成好几行），跟着页面滚走，由这条浮条接手。关掉下载弹窗后，键盘焦点回到打开它的「下载选中」；整批下载完、退出选择模式时回到「批量下载」。</li>
                <li><strong>成批选择</strong>：选择条上的<strong>全选列表</strong>、<strong>全选当前图</strong>（散点图打开时出现）与「<strong>前 N 场 · 按当前排序选择</strong>」都会替换已有选择（N 超过列表场数就按全部算），「清空」一键取消。<strong>已选什么时候保留、什么时候清掉</strong>：切换排序、切换散点图或视图都保留；改变操作时间段只移除范围外的那几场（恢复全部范围也不会找回来）；换登录账号时清空并退出选择模式；因当前排序口径缺值而暂时不在列表里的已选战役，选择条上会报数，它们排在下载队列末尾。</li>
                <li><strong>退出</strong>：再点「退出选择」或按 <strong>Esc</strong>（刚勾完勾选框、焦点还停在框上时也行；只有在「前 N 场」里打字时 Esc 不退出），已选保留，再次进入可以接着用；退出后键盘焦点回到「批量下载」（在卡片勾选框上按 Esc 时回到这张卡片的展开按钮），不会掉到页首。整批都下载完、关闭弹窗后自动退回普通浏览。选择模式之外，点卡片、点散点照旧进入战役详情。</li>
                <li><strong>下载选中</strong>打开设置：勾选要画进图里的<strong>战役原数据、盈亏概览、操作日情绪日记、K 线盘面、完整 Legs 列表</strong>（至少一项；情绪日记是私人记录，分享前可以取消，没有日记的战役不画这一块）；K 线周期默认<strong>自动</strong>（默认 5 分钟线，某场的视窗或拉取量放不下时自动放宽，与详情页首屏同一套选法；详情页里已保存的反事实分支会把首屏视窗撑宽、周期随之变粗，批量盘面只画战役本身，不计入分支，这类战役的自动周期可能比详情页首屏细），也可统一指定 1 分钟、5 分钟、15 分钟或 1 小时。<strong>周期只管盘面</strong>：盈亏概览（峰值浮盈等）与详情页一样按自动周期的那一份 K 线计算、读数与详情页逐位一致（详情页选中的反事实分支越出 Legs 的时间跨度时除外：它会把详情页算读数的那份 K 线窗口一并撑宽，在详情页取消选中该分支后两边一致），不随这里选的周期变；盘面周期与它不同时每场多拉一份 K 线。没画 K 线盘面时周期不起作用，这一组调暗停用，进度区也不写周期。<strong>指定周期是下限</strong>：盘面按所选视窗倍数取景，某场按所选周期超过 1000 根（每根不到 1.4 像素）时才放宽到放得下的周期，这一行在队列里标出实际周期，图里盘面标题写的也是实际周期；较短的战役按所选周期照画。<strong>盘面视窗</strong>可选 1.1x、2.1x、3.1x、5x、11x、21x、31x、41x、51x（与详情页 K 线上方那一排倍数按钮同一组），<strong>默认 1.1 倍</strong>：盘面显示战役时长的几倍、战役居中，1.1 倍几乎只放战役本身，倍数越大前后行情越多；不沿用详情页里手动拖动的视窗，没画 K 线盘面时这一项停用。</li>
                <li><strong>逐场生成</strong>：每场都用详情页同一套计算在后台出图（同一份平仓价校正、同一份盈亏概览），弹窗里逐行显示等待 / 生成中 / 已生成 / 失败及原因。这是<strong>只读操作</strong>，不修改成交、战役或反事实记录，也不触发落库自愈；情绪日记一批只读一次，不回写本机、不触发云端同步。一批之内导出时刻、复盘时钟与账户级 DSI/USI 样本只取一次，每张图口径一致。账户里有某场战役读不出来（重读一次仍失败）时不会拖垮整批：样本按其余场次汇总，图中「盈亏概览」下注明缺了哪几场、「不对称风险贡献」按几场计算，弹窗里也提示一次；一场样本都读不出（多半是断网）才记为失败。唯一的例外：缺的正是某场自己（它当时读不出、重试后读出来了），它那张图单独重取一次样本，不会写「缺本场」，读数与详情页一致；其余图保持各自的注明。平仓价核验时 K 线接口出错、盘面 K 线加载失败或单场超过 150 秒记为失败，其余照常继续，失败项可单场重试或一次全部重试；可随时暂停、继续。本机查不到成交记录（换了浏览器、清过历史成交）的战役不算失败，与详情页一样按腿快照出图。生成时请停留在本页：切到其他标签页时浏览器会暂停绘图，回来后接着生成，离开的时间不计入 150 秒。<strong>交易所没有这段 K 线</strong>的战役（上线前、已下架的老合约）不算失败：照常出图并在队列里标「无 K 线」，画了 K 线盘面的在盘面位置写明原因；这时「峰值浮盈」缺少 K 线路径，只能按已实现盈亏兜底——只画盈亏概览、没画盘面时，这句写在「盈亏概览」下面，进度区同样报场数。</li>
                <li><strong>打包</strong>：每场一张 PNG，文件名前加按当前排序的序号（001、002…），打包成 <strong>交易战役_日期-时间_N张.zip</strong>。已生成的图片累计达到 <strong>256 MB</strong> 时自动暂停分包：点「下载第 1 包并继续」后接着生成剩下的战役，包名带「第 N 包」，序号全批连续；分过包之后，每次下载都是新的一包，只装还没下载过的图片、包号顺延（例如最后一包下完再重试补上的一场，另打成下一包，不会把已下载的图片再装一遍）。没分过包时，重试补上几场后再下载，得到的是装着全部图片的一份完整 ZIP。生成中或还有没下载的图片时，关闭弹窗会先在弹窗里确认。图的内容按勾选走：没画 K 线盘面时，标题行与「战役原数据」里不写 K 线周期；K 线盘面固定按浅色主题画，与浅色看板一致（应用是深色主题也一样）。ZIP 里的中文文件名在访达、归档实用工具与命令行 unzip 解压后都正确。</li>
              </ul>
              ...
              <ul className="list-disc pl-6 text-[14px] text-foreground/90 space-y-1">
                ...
                <li>反事实盘面以原始战役的 K 线、真实事件标记、竖线、区间线和委托空单为只读背景，再叠加紫色虚拟轨迹；不会为了显示反事实而丢掉原始盘面信息。<strong>反事实盘面与原始盘面同高</strong>：原始盘面按可视区实测的高度拉满，反事实盘面用同一个高度，随窗口一起变。它与原始盘面共用完整的 51 倍数据范围和快捷档位，但默认从 <strong>1.1 倍</strong>开始，在完整战役内容外保留少量上下文。<strong>反事实盘面的周期按它自己的视窗选</strong>：同样默认 5 分钟线、放不下时自动放宽，只看它自己的倍数——原始盘面切到 51 倍不会把它也放宽；手动点过的周期两块盘面共用。与计算用那一份，或与倍率视图下的原始盘面那一份同周期时直接共用，不另拉（原始盘面在「1天 / 1周 / 1月」预设下拉的是更宽的窗口，不共用）。</li>
                ...
                <li><strong>「币量 / 仓位」一列</strong>：上行是<strong>币量</strong>（可编辑，= 名义 ÷ 开仓价），下行小字是 USD 名义仓位，与原始 Legs 表的「币量 / 仓位」同写法同数。改币量时名义 = 币量 × 开仓价；<strong>只改开仓价（输入框或拖开仓线）时币量不变</strong>，名义随价变，这条腿的盈亏变化 = 币量 × 价差。改动摘要里写「币量 旧 → 新」，只改开仓价不会多出一条币量。没动过的腿原样重跑，与实际逐位相同。</li>
              </ul>
            ...
            <section id="s4-5" className="scroll-mt-20">
              ...
              <P><strong>「规则」页只做一件事：一条一行列出规则。</strong>每条左边是时间、中间是规则原文、右边是来源战役的币种，点它跳到那场交易战役（没有来源战役的写「无来源战役」）。右上角可按<strong>操作时间</strong>（默认，来源战役的操作时间，与交易战役列表同一个时间）或<strong>创建时间</strong>排序，再点一次切换从新到旧 / 从旧到新；按操作时间排时，没有来源战役的规则排在最后。上面说的类型、权重、演化等级、激活 / checklist / 必填这些属性仍按原样保存、照旧在开仓 checklist 与必填提醒里起作用，只是不在规则页上呈现与修改。</P>
              <Highlight>
