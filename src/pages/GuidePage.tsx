...
P 不再一把手填，而是拆成三个更可回答的问题：<strong>「这个结构还活得下去吗（P₁）」</strong>、<strong>「假如它活着，价格能走到 T 吗（P₂）」</strong>、<strong>「假如它死了，价格还是走到了 T 吗（P₃）」</strong>。P₂ 与 P₃ 都必须按<strong>条件概率</strong>来估——先假定结构的死活，再问突破的把握，否则会把结构风险重复计价。
          ...
          <section id="s4" className="scroll-mt-20">
            ...
            <section id="s4-3" className="scroll-mt-20">
              ...
              <P>默认按<strong>真实操作时间</strong>从新到旧排序。排序行依次是操作时间、镜像止盈，一条短分隔线之后是<strong>预期回撤、涨幅、涨幅效率、盈亏比、加仓效用、几何期望、算术期望</strong>七项排在一起（与封面上镜像止盈之后的七格同序），再一条短分隔线之后是 DSI 贡献、USI 贡献、杠杆倍数、重要性与字母（重要性放在后面，排在字母之前）；每一项都可双向排序。排序行<strong>左对齐</strong>、按钮依次排开、间距均匀，放不下时整行换行。选中的那一项在 Σ 的位置换成方向箭头；没有公式的操作时间、杠杆倍数、字母平时也留着同样宽的空位，切换排序时按钮不会换位。涨幅、涨幅效率、加仓效用与其他公式指标一样，<strong>双击或右键</strong>打开公式浮层：公式、每个符号的含义、一个代入数字的例子、哪些战役不参与，底部是「查看散点图」入口。<strong>涨幅</strong>按战役算，按主力方向计，空单价格跌了为正：<strong>开仓价取主力各笔里最有利的那个</strong>（主多最低、主空最高）；<strong>平仓价看主力平仓那一刻有没有对冲把行情锁住</strong>——滚动对冲仍持有、或与主力同一次操作里平掉（相差不超过一分钟），以及<strong>已触发的初始对冲 A/B、回场对冲与主力同一次操作里平掉</strong>（主力平了它们还挂着不算），都算锁住：有就取其中最早开的那张对冲的开仓价，对冲一挂上主力后面的行情就被锁住了；没有就取主力自己的平仓价（最后平的那几笔里最有利的）。盈亏概览「涨幅」的 ⓘ 里写明本场取的两个价和来自哪张腿。初始对冲 A/B 不算；开平价与详情页 Legs 表同源（含平仓价校正）。涨幅效率、加仓效用、散点图、盈亏概览与反事实都从这一个数派生；主力都还没平仓的战役不进入这一档排序，卡片第二行常驻显示这个数（紧跟「预期回撤」）。<strong>涨幅效率 = 主力涨幅 ÷ 预期回撤</strong>（两者都是价格层面的百分数，结果是倍数）：主力涨了 12%、入场到对冲边界 4%，效率就是 +3.00——价格走出了 3 个「预期回撤」。主力未平仓或算不出预期回撤的战役不进入这一档排序；卡片上紧跟「涨幅」显示，悬停给出代入值。<strong>加仓效用 = 盈亏比 ÷ 涨幅效率</strong>：只拿主力、不加仓时，盈亏比大致就是主力的涨幅效率（最大预期亏损按入场到对冲边界的距离定），比值约为 1；大于 1 说明加仓把同一段行情放大成了更多的 R，小于 1 说明加仓、对冲或止盈吃掉了行情。<strong>只在做过加仓、且涨幅效率为正时计算</strong>（有一条成交过的加仓腿；没有加仓时这个比值恒在 1 附近，没有信息量；涨幅效率为负时亏损战役负负得正会排到最前，接近 0 时分母过小会把主力几乎没动的战役放大成十几倍，所以按显示到两位小数的值判，0.00 也不算），其余战役不进入这一档排序与散点图；卡片上紧跟「盈亏比」显示，算不出时显示「—」。这里的操作时间是客观发生时间，不是无知之幕时间机器里的模拟时间。</P>
              ...
              <P>盈亏比、预期回撤、涨幅、涨幅效率、加仓效用、算术期望、几何期望、重要性、镜像止盈、DSI 贡献、USI 贡献都各自配有一张散点图。在排序行上<strong>双击或右键</strong>对应指标打开公式浮层，浮层底部的「查看散点图」即可展开；再点一次「收起散点图」关闭。<strong>盈亏比、涨幅、涨幅效率、加仓效用、算术期望与几何期望默认展开的是分布图、镜像止盈默认展开的是柱状图</strong>，其余指标以及这几项切回「时序」后，横轴都是按<strong>客观操作时间</strong>排列的战役序号，与列表当前排序无关——改排序不会让点位移动。</P>
              <ul className="list-disc pl-6 text-[14px] text-foreground/90 space-y-1">
                <li><strong>颜色分两类。</strong>本身带盈亏方向的指标（盈亏比、涨幅、涨幅效率、加仓效用、算术期望、几何期望）按数值正负着色：正绿、负红、零灰。不带方向的纯量级指标用单色，避免暗示盈亏：重要性琥珀、DSI 贡献红、USI 贡献绿。</li>
                <li><strong>涨幅、涨幅效率、加仓效用、算术期望默认看分布</strong>，与盈亏比同一套画法：横轴就是指标本身（涨幅 %、涨幅效率与加仓效用「倍」、算术期望 R，线性刻度），纵轴是落在该值附近的战役数量——点按档从底线向上堆叠，灰色曲线是密度估计，不考虑时间先后。灰色 0 线分开正负：涨幅与涨幅效率的 0 叫「不涨不跌」，摘要条报「顺向」占比（价格朝主力方向走的场数）；算术期望的 0R 是盈亏平衡（对应 b = +1R），摘要条报「正期望」占比；加仓效用另有一条<strong>琥珀色 1.00 虚线</strong>「加仓没有额外放大」，摘要条多报一项「放大（&gt; 1）」占比。0 与 1.00 都是档边界，点不会吸附到线的另一侧，恰好落在线上的归右侧；窗口取 p2–p98 并一定把 0（和 1.00）圈进来，超出的极端值贴边画成三角并计数。用面板右上角的「时序 | 分布」切回时序：纵轴是同一个数，横轴按客观操作时间排列。与排序同一组函数取数：主力未平仓的不进涨幅图，还算不出预期回撤的不进涨幅效率图，<strong>没有加仓、或涨幅效率不为正的战役不进加仓效用图</strong>，算不出盈亏比的不进算术期望图；未进图的场数写在图下脚注里。</li>
