...
P 不再一把手填，而是拆成三个更可回答的问题：<strong>「这个结构还活得下去吗（P₁）」</strong>、<strong>「假如它活着，价格能走到 T 吗（P₂）」</strong>、<strong>「假如它死了，价格还是走到了 T 吗（P₃）」</strong>。P₂ 与 P₃ 都必须按<strong>条件概率</strong>来估——先假定结构的死活，再问突破的把握，否则会把结构风险重复计价。
          ...
          <section id="s4" className="scroll-mt-20">
            ...
            <section id="s4-3" className="scroll-mt-20">
              ...
              <P>战役卡片默认只保留两层核心信息：第一层是标题、方向、标的、杠杆倍数、唯一编号、<strong>真实操作时间</strong>、重要性与结束状态；第二层是预期回撤、涨幅、涨幅效率、盈亏比、加仓效率、单场算术期望、单场几何期望与镜像止盈状态（涨幅效率紧跟它的两个输入「预期回撤 / 涨幅」，加仓效率紧跟「盈亏比」）。「机会质量」已删掉：它用 max（b, 1）÷ 预期回撤把亏损场一律抹成同一个数，涨幅效率（主力涨幅 ÷ 预期回撤）衡量同一件事更直接。指标为正时使用绿色、为负时使用红色，无法计算时显示「—」。</P>
              ...
              <P>「统计概览」最前面是<strong>操作时间段</strong>筛选，默认<strong>全部</strong>：它决定后面每一个数的取样范围，按<strong>客观操作时间</strong>切（不受时间机器的模拟时钟影响），起止两天都算在内；可用「近 7 / 30 / 90 天、今年」等预设，也可以自己填起止日期，所选范围写进地址栏，从详情页返回时会跟着回来。<strong>统计概览、战役卡片与散点图读的是同一批战役</strong>，所以会一起跟着收窄——不会出现「统计说 45 场、下面却躺着 230 张卡片」。一旦设了范围，缺少客观操作时间的战役无从安放，会被排除并在浮层里报出场数；批量结束进行中战役等写库操作不受筛选影响，仍然针对全部战役。概览本身汇总有效战役、镜像止盈、胜率、平均盈亏比、期望值、几何期望与不对称风险；排序行只负责改变战役顺序，避免统计与操作混在一起。</P>
              ...
              <P>默认按<strong>真实操作时间</strong>从新到旧排序；还可以按重要性、预期回撤、盈亏比、算术期望、几何期望、镜像止盈、DSI 贡献、USI 贡献、杠杆倍数、<strong>涨幅</strong>、<strong>涨幅效率</strong>、<strong>加仓效率</strong>或字母顺序双向排序。<strong>涨幅</strong>取主力那条腿的涨跌幅——与详情页 Legs 表「涨跌幅」列同一个数：同一对开平价（含平仓价校正）、按主力方向计，空单价格跌了为正；主力还没平仓的战役不进入这一档排序，卡片第二行常驻显示这个数（紧跟「预期回撤」）。<strong>涨幅效率 = 主力涨幅 ÷ 预期回撤</strong>（两者都是价格层面的百分数，结果是倍数）：主力涨了 12%、入场到对冲边界 4%，效率就是 +3.00——价格走出了 3 个「预期回撤」。主力未平仓或算不出预期回撤的战役不进入这一档排序；卡片上紧跟「涨幅」显示，悬停给出代入值。<strong>加仓效率 = 盈亏比 ÷ 涨幅效率</strong>：只拿主力、不加仓时，盈亏比大致就是主力的涨幅效率（最大预期亏损按入场到对冲边界的距离定），比值约为 1；大于 1 说明加仓把同一段行情放大成了更多的 R，小于 1 说明加仓、对冲或止盈吃掉了行情。<strong>只对做过加仓的战役计算</strong>（有一条成交过的加仓腿；没有加仓时这个比值恒在 1 附近，没有信息量），没有加仓、算不出盈亏比或涨幅效率、或涨幅效率为 0 的战役不进入这一档排序；卡片上紧跟「盈亏比」显示，算不出时显示「—」。这里的操作时间是客观发生时间，不是无知之幕时间机器里的模拟时间。</P>
              ...
              <P>盈亏比、预期回撤、算术期望、几何期望、重要性、镜像止盈、DSI 贡献、USI 贡献都各自配有一张散点图。在排序行上<strong>双击或右键</strong>对应指标打开公式浮层，浮层底部的「查看散点图」即可展开；再点一次「收起散点图」关闭。<strong>盈亏比与几何期望默认展开的是分布图、镜像止盈默认展开的是柱状图</strong>，其余指标以及这三者切回「时序」后，横轴都是按<strong>客观操作时间</strong>排列的战役序号，与列表当前排序无关——改排序不会让点位移动。</P>
              <ul className="list-disc pl-6 text-[14px] text-foreground/90 space-y-1">
                <li><strong>颜色分两类。</strong>本身带盈亏方向的指标（盈亏比、算术期望、几何期望）按数值正负着色：正绿、负红、零灰。不带方向的纯量级指标用单色，避免暗示盈亏：重要性琥珀、DSI 贡献红、USI 贡献绿。</li>
