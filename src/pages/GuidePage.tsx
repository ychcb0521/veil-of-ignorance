...
P 不再一把手填，而是拆成三个更可回答的问题：<strong>「这个结构还活得下去吗（P₁）」</strong>、<strong>「假如它活着，价格能走到 T 吗（P₂）」</strong>、<strong>「假如它死了，价格还是走到了 T 吗（P₃）」</strong>。P₂ 与 P₃ 都必须按<strong>条件概率</strong>来估——先假定结构的死活，再问突破的把握，否则会把结构风险重复计价。
            ...
            <section id="s3-1c" className="scroll-mt-20">
              ...
              <P>
                ...
                <strong>A 本账 · 浮盈垫</strong>给出 X₂、b、P₀、加仓后均价与 S₁ 处的对冲量，落袋利润永远不进这本账；
                <strong>B 本账 · 落袋镜像</strong>（可选）。<strong>已落袋的镜像利润同样是一块垫子</strong>——两本账用的是<strong>同一个公式</strong>，只是垫子来源不同：<span className="font-mono">加仓量 = 垫 ÷ 险</span>，险 = |S₂ − S₁|；A 的垫是浮盈 Y₁ = X₁(S₁−S̄)（X₂ = Y₁ ÷ 险，与 X₁ ÷ b 恒等），B 的垫是已落袋的 G（<span className="font-mono">X_G = G ÷ 险</span>）。取 K<sub>B</sub> = S₁ 就是这个<strong>零风险档</strong>：跌回 S₁ 时 A 本账归零、B 腿恰好亏掉 G 而 G 已在口袋里，全场对本金零风险，代价是把落袋还回去；此时对冲要一并扛起 B 腿，<span className="font-mono">对冲 @ S₁ = X₁ + X₂ + X_G</span>。把 K<sub>B</sub> 拖到 S₁ 之下才产生敞口——给它自己的零风险线 K<sub>B</sub>，使 X₂ᴮ 从 S₂ 跌到 K<sub>B</sub> 恰好亏掉 G（U 本位 X₂ᴮ = G ÷ |S₂ − K<sub>B</sub>|；币本位 G 以币计、X₂ᴮ = G·K<sub>B</sub> ÷ |S₂ − K<sub>B</sub>|）。
