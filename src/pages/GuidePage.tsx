...
P 不再一把手填，而是拆成三个更可回答的问题：<strong>「这个结构还活得下去吗（P₁）」</strong>、<strong>「假如它活着，价格能走到 T 吗（P₂）」</strong>、<strong>「假如它死了，价格还是走到了 T 吗（P₃）」</strong>。P₂ 与 P₃ 都必须按<strong>条件概率</strong>来估——先假定结构的死活，再问突破的把握，否则会把结构风险重复计价。
          ...
          <section id="s4" className="scroll-mt-20">
            ...
            <section id="s4-3" className="scroll-mt-20">
              ...
              <P>战役卡片默认只保留两层核心信息：第一层是标题、方向、标的、杠杆倍数、唯一编号、<strong>真实操作时间</strong>、重要性与结束状态；第二层是预期回撤、涨幅、涨幅效率、盈亏比、加仓效率、单场几何期望、单场算术期望与镜像止盈状态。窗口宽度 ≥ 1280px 时第二层按列排：<strong>涨幅、涨幅效率、盈亏比、加仓效率、几何期望五格与上方排序行的同名按钮共用同一套列，左缘落在同一条竖线上</strong>——排序行就是这几列的表头，每格左侧一条细分隔线；五格之后的算术期望、镜像止盈也各占一列，同样压在排序行同名按钮的竖线上；预期回撤（涨幅效率的分母）在首列、与「排序方式」同一起点。每一格在所有卡片上都落在同一列。列宽按真实最长的读数定（如盈亏比「49628.76%（496.29）」、几何期望带「仓位击穿」徽标），数字用等宽字体；更极端的读数在本格内以省略号收住、不压到下一列，悬停可看完整数值。窗口在 1280px 与约 1414px 之间时，排序行末尾的杠杆倍数、重要性、字母放不下，会在最后一列里换到第二行，列线照旧对齐；更宽时一行放下。窗口窄于 1280px 时放不下这一排，排序行与卡片都按原顺序自然换行，不再强求对齐。「机会质量」已删掉：它用 max（b, 1）÷ 预期回撤把亏损场一律抹成同一个数，涨幅效率（主力涨幅 ÷ 预期回撤）衡量同一件事更直接。指标为正时使用绿色、为负时使用红色，无法计算时显示「—」。</P>
              ...
              <P>默认按<strong>真实操作时间</strong>从新到旧排序。排序行依次是操作时间，紧接着<strong>涨幅、涨幅效率、盈亏比、加仓效率、几何期望</strong>五项排在一起（与卡片上的同名五格逐列对齐），一条短分隔线之后是预期回撤、算术期望、镜像止盈、DSI 贡献、USI 贡献、杠杆倍数、重要性与字母（重要性放在后面，排在字母之前；算术期望、镜像止盈也压在卡片同名格的竖线上）；每一项都可双向排序。选中的那一项在 Σ 的位置换成方向箭头；没有公式的操作时间、杠杆倍数、字母在窄屏上平时也留着同样宽的空位，切换排序时按钮不会换位（宽屏上这三项选中时多出箭头，列宽已按选中时留足，不会挤到别的列）。涨幅、涨幅效率、加仓效率与其他公式指标一样，<strong>双击或右键</strong>打开公式浮层：公式、每个符号的含义、一个代入数字的例子、哪些战役不参与，底部是「查看散点图」入口。<strong>涨幅</strong>取主力那条腿的涨跌幅——与详情页 Legs 表「涨跌幅」列同一个数：同一对开平价（含平仓价校正）、按主力方向计，空单价格跌了为正；主力还没平仓的战役不进入这一档排序，卡片第二行常驻显示这个数（紧跟「预期回撤」）。<strong>涨幅效率 = 主力涨幅 ÷ 预期回撤</strong>（两者都是价格层面的百分数，结果是倍数）：主力涨了 12%、入场到对冲边界 4%，效率就是 +3.00——价格走出了 3 个「预期回撤」。主力未平仓或算不出预期回撤的战役不进入这一档排序；卡片上紧跟「涨幅」显示，悬停给出代入值。<strong>加仓效率 = 盈亏比 ÷ 涨幅效率</strong>：只拿主力、不加仓时，盈亏比大致就是主力的涨幅效率（最大预期亏损按入场到对冲边界的距离定），比值约为 1；大于 1 说明加仓把同一段行情放大成了更多的 R，小于 1 说明加仓、对冲或止盈吃掉了行情。<strong>只对做过加仓的战役计算</strong>（有一条成交过的加仓腿；没有加仓时这个比值恒在 1 附近，没有信息量），没有加仓、算不出盈亏比或涨幅效率、或涨幅效率为 0 的战役不进入这一档排序；卡片上紧跟「盈亏比」显示，算不出时显示「—」。这里的操作时间是客观发生时间，不是无知之幕时间机器里的模拟时间。</P>
              ...
              <P>盈亏比、预期回撤、涨幅、涨幅效率、加仓效率、算术期望、几何期望、重要性、镜像止盈、DSI 贡献、USI 贡献都各自配有一张散点图。在排序行上<strong>双击或右键</strong>对应指标打开公式浮层，浮层底部的「查看散点图」即可展开；再点一次「收起散点图」关闭。<strong>盈亏比与几何期望默认展开的是分布图、镜像止盈默认展开的是柱状图</strong>，其余指标以及这三者切回「时序」后，横轴都是按<strong>客观操作时间</strong>排列的战役序号，与列表当前排序无关——改排序不会让点位移动。</P>
              <ul className="list-disc pl-6 text-[14px] text-foreground/90 space-y-1">
                ...
                <li><strong>涨幅、涨幅效率、加仓效率三张图。</strong>纵轴分别是主力涨幅（%，按主力方向计，与 Legs 表「涨跌幅」同一个数）、涨幅效率（倍，= 涨幅 ÷ 预期回撤）与加仓效率（倍，= 盈亏比 ÷ 涨幅效率），横轴按客观操作时间排列，灰色零线分开正负。与排序同一组函数取数：主力未平仓的不进涨幅图，还算不出预期回撤的不进涨幅效率图，<strong>没有加仓的战役不进加仓效率图</strong>；读加仓效率时，1 是「加仓没有额外放大」的参照。未进图的场数写在图下脚注里。</li>
