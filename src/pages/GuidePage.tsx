...
P 不再一把手填，而是拆成三个更可回答的问题：<strong>「这个结构还活得下去吗（P₁）」</strong>、<strong>「假如它活着，价格能走到 T 吗（P₂）」</strong>、<strong>「假如它死了，价格还是走到了 T 吗（P₃）」</strong>。P₂ 与 P₃ 都必须按<strong>条件概率</strong>来估——先假定结构的死活，再问突破的把握，否则会把结构风险重复计价。
          ...
          <section id="s4" className="scroll-mt-20">
            ...
            <section id="s4-3" className="scroll-mt-20">
              ...
              <P>战役卡片默认只保留两层核心信息：第一层是标题、方向、标的、杠杆倍数、唯一编号、<strong>真实操作时间</strong>、重要性与结束状态；第二层是预期回撤、涨幅、涨幅效率、机会质量、盈亏比、加仓效率、单场算术期望、单场几何期望与镜像止盈状态（涨幅效率紧跟它的两个输入「预期回撤 / 涨幅」，加仓效率紧跟「盈亏比」）。指标为正时使用绿色、为负时使用红色，无法计算时显示「—」。</P>
              ...
              <P>默认按<strong>真实操作时间</strong>从新到旧排序；还可以按重要性、预期回撤、机会质量、盈亏比、算术期望、几何期望、镜像止盈、DSI 贡献、USI 贡献、杠杆倍数、<strong>涨幅</strong>、<strong>涨幅效率</strong>、<strong>加仓效率</strong>或字母顺序双向排序。<strong>涨幅</strong>取主力那条腿的涨跌幅——与详情页 Legs 表「涨跌幅」列同一个数：同一对开平价（含平仓价校正）、按主力方向计，空单价格跌了为正；主力还没平仓的战役不进入这一档排序，卡片第二行常驻显示这个数（紧跟「预期回撤」）。<strong>涨幅效率 = 主力涨幅 ÷ 预期回撤</strong>（两者都是价格层面的百分数，结果是倍数）：主力涨了 12%、入场到对冲边界 4%，效率就是 +3.00——价格走出了 3 个「预期回撤」。主力未平仓或算不出预期回撤的战役不进入这一档排序；卡片上紧跟「涨幅」显示，悬停给出代入值。<strong>加仓效率 = 盈亏比 ÷ 涨幅效率</strong>：只拿主力、不加仓时，盈亏比大致就是主力的涨幅效率（最大预期亏损按入场到对冲边界的距离定），比值约为 1；大于 1 说明加仓把同一段行情放大成了更多的 R，小于 1 说明加仓、对冲或止盈吃掉了行情。算不出盈亏比或涨幅效率、或涨幅效率为 0 的战役不进入这一档排序；卡片上紧跟「盈亏比」显示，算不出时显示「—」。这里的操作时间是客观发生时间，不是无知之幕时间机器里的模拟时间。</P>
