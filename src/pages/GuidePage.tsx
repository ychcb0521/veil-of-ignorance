...
P 不再一把手填，而是拆成三个更可回答的问题：<strong>「这个结构还活得下去吗（P₁）」</strong>、<strong>「假如它活着，价格能走到 T 吗（P₂）」</strong>、<strong>「假如它死了，价格还是走到了 T 吗（P₃）」</strong>。P₂ 与 P₃ 都必须按<strong>条件概率</strong>来估——先假定结构的死活，再问突破的把握，否则会把结构风险重复计价。
            ...
            <section id="s3-1c" className="scroll-mt-20">
              ...
              <P>
                <strong>顶栏「加仓」按钮就是这套公式的计算器</strong>（在「倒叙播放」左侧）。X₁ / S̄ 按各腿开仓价折算后从持仓读入、S₂ 读现价，三者都可手改并一键复位；S₁ 必须手填——那是你的判断。结果分<strong>两本账</strong>，互不掺和：
                <strong>A 本账 · 浮盈垫</strong>给出 X₂、b、新腿占比、加仓后均价与 S₁ 处的对冲量，落袋利润永远不进这本账；
                <strong>B 本账 · 落袋镜像</strong>（可选）。<strong>已落袋的镜像利润同样是一块垫子</strong>——两本账用的是<strong>同一个公式</strong>，只是垫子来源不同：<span className="font-mono">加仓量 = 垫 ÷ 险</span>，险 = |S₂ − S₁|；A 的垫是浮盈 Y₁ = X₁(S₁−S̄)（X₂ = Y₁ ÷ 险，与 X₁ ÷ b 恒等），B 的垫是已落袋的 G（<span className="font-mono">X_G = G ÷ 险</span>）。X_G 与 X₂ <strong>同级</strong>——用的是<strong>同一条 S₁、同一个 S₂</strong>，连风险距离都是同一个「险」，差别只在垫子从浮盈换成了落袋，所以默认直接相加、对冲也一并扛起。K<sub>B</sub> 是一个<strong>可选旋钮</strong>，留空即取 S₁，也就是这个<strong>零风险档</strong>：跌回 S₁ 时 A 本账归零、B 腿恰好亏掉 G 而 G 已在口袋里，全场对本金零风险，代价是把落袋还回去；此时对冲要一并扛起 B 腿，<span className="font-mono">对冲 @ S₁ = X₁ + X₂ + X_G</span>。把 K<sub>B</sub> 拖到 S₁ 之下才产生敞口——给它自己的零风险线 K<sub>B</sub>，使 X₂ᴮ 从 S₂ 跌到 K<sub>B</sub> 恰好亏掉 G（U 本位 X₂ᴮ = G ÷ |S₂ − K<sub>B</sub>|；币本位 G 以币计、X₂ᴮ = G·K<sub>B</sub> ÷ |S₂ − K<sub>B</sub>|）。
              </P>
              <RedHighlight>
                ...
                （多下多少币、走到那条线时账面多少钱），并给一个「按盘口重算」的一键改正。
                <br /><br />
                <strong>另一个同源的坑</strong>：B 本账一开，A 段的「加仓 X₂」与「对冲 @ S₁」<strong>就只是一半</strong>。
                真正要下的是 <span className="font-mono">合计加仓 = X₂ + X_G</span>、
                真正要挂的是 <span className="font-mono">合计对冲 = X₁ + X₂ + X_G</span>——
                这两个数现在都以大字呈现，A 段那两个会标上「仅 A」。照 A 段的数去挂对冲，等于只对冲了一部分仓位。
              </RedHighlight>
              <P>
                定线推仓、定仓推线两个旋钮互为反函数；K<sub>B</sub> 拖到 S₁ 之下即「零风险线更低」，跌到 S₁ 只吃掉 G 的一部分，界面会标出吃掉多少、剩多少。
                G 不填则 B 本账关闭，A 本账一字不变。系统会检测本场「止盈1」落袋作为建议值，点一下才填入，不自动代入。「本场」同时看两只钟：模拟平仓时间不早于当前持仓开仓，且<strong>操作时间（真实平仓时刻）不早于当前持仓最早一笔成交的真实开仓时刻</strong>——同一段历史重放多遍时，别的重放在同一模拟时刻落袋的止盈不会混进 G，被排除的笔数（含没有操作时间的老记录）在下方以小字注明（持仓里只要有一笔成交没有真实开仓时刻——老仓位，或在老仓位上新加的一刀——起点无从确定，仍只看模拟时间）。落袋之后又加过仓（按成交笔数算，合并进同一仓位的加仓也算）时，按钮转黄并提示 G 很可能已经花掉。计算器右上角那个几乎看不见的「?」展开公式速览并链回本节。弹窗里只放数字，判断规则都写在这里：<strong>X₂ 是上限不是目标</strong>——取等号时综合成本恰好落在 S₁ 上，要留垫子就取小于 X₂；<strong>止损线每上移一次都要重算</strong>，b 由这三个价格定义，换一组就变。输入区下方那条<strong>价格阶梯</strong>把 S̄ / S₁ / S₂ 按比例画在一条轴上，绿色段是浮盈垫 (S̄→S₁)、主色段是新腿风险距离 (S₁→S₂)，b 就是两段之比——若 S₁ 还在成本线下方（尚处浮亏），阶梯上一眼看得出没有绿色段，此时无解并非故障。对冲分两处：A 线 S₁ 放反向的 X₁ + X₂；B 腿单独在 K<sub>B</sub> 处对冲 X₂ᴮ——B 腿亏掉的是落袋利润，不碰浮盈垫，两本账各自归零、互不借钱。
              </P>
          ...
          <section id="s4" className="scroll-mt-20">
            ...
            <section id="s4-3" className="scroll-mt-20">
              ...
              <ul className="list-disc pl-6 text-[14px] text-foreground/90 space-y-1">
                ...
                <li><strong>Legs 列表每条腿都标明开仓/平仓时间、开仓价/平仓价、仓位与状态</strong>；还没平仓的腿，平仓时间与平仓价显示「—」。「委托」列按真实业务归属呈现：委托先按挂出时刻归到当时开着的那笔主力；<strong>加仓之后挂出的委托放在加仓那一行的后面，再加仓就放到最新那次加仓的行后面</strong>（按开仓时刻排，不看「加仓1 / 加仓2」的编号；加仓开出前 5 分钟内预挂、且加仓开出时仍挂着的也算这次加仓；主力开仓前预挂的仍归主力；加仓先平掉之后挂出的回到主力；反过来主力先平、加仓还开着时仍接在加仓后面；全部平完之后才挂出的，与收尾前一刻挂出的落在同一行）。这只是 Legs 表与导出 PNG 的<strong>展示位置</strong>，初始暴露、预期最大亏损等风险指标仍以主力为锚。对冲触发后的反向委托放在对应的对冲 leg；镜像止盈行则显示止盈挂单时间与触发时间，避免把主力委托误记到镜像腿。</li>
