          ...
          <section id="s4" className="scroll-mt-20">
            ...
            <section id="s4-3" className="scroll-mt-20">
              ...
              <ul className="list-disc pl-6 text-[14px] text-foreground/90 space-y-1">
                ...
                <li><strong>Legs 列表每条腿都标明开仓/平仓时间、开仓价/平仓价、仓位与状态</strong>；还没平仓的腿，平仓时间与平仓价显示「—」。平仓价右侧的<strong>「涨跌幅」</strong>按持仓方向计（多单 =（平仓价 − 开仓价）÷ 开仓价，空单 =（开仓价 − 平仓价）÷ 开仓价），正数即这条腿在价格上占优、按所示的这一对开平价看与盈亏同号；与左边两格用同一对价（含 K 线平仓价校正），未平仓显示「—」；一个仓位分几刀平掉时平仓价取最后一刀、盈亏是各刀合计，两者符号可能不同；主力阶段子行按主力方向、各自起止价各算各的，合计行留空。PNG 导出同列。「币量 / 仓位」右侧的<strong>「占比」</strong>上行是这条腿的币量占本场各腿币量合计的百分比、下行是名义仓位占各腿名义仓位合计的百分比，两个分母各算各的，用的是「币量 / 仓位」列每行背后的同一组数，按未舍入的原值相加（各行与合计行各自取两位小数，把各行印出来的数手工相加，末位可能与合计行对不上）；状态为「挂单中」的对冲 / 镜像腿（还没有成交或平仓记录）不计入合计、两行都显示「—」，缺开仓价的腿上行显示「—」、不进币量合计；各行分别取一位小数，逐行相加不一定恰好是 100.0%（腿越多、舍入差可能越大，如六条等额的腿各印 16.7%，加起来是 100.2%）；阶段子行留空，合计行在「币量 / 仓位」格写出这两个分母（Σ币量 / Σ名义仓位）、在「占比」格写 100.0%，一条腿都不计入时这两格都显示「—」（计入的腿都缺开仓价时只有上行是「—」）。PNG 导出同列。「委托」列按真实业务归属呈现：委托先按挂出时刻归到当时开着的那笔主力；<strong>加仓之后挂出的委托放在加仓那一行的后面，再加仓就放到最新那次加仓的行后面</strong>（按开仓时刻排，不看「加仓1 / 加仓2」的编号；加仓开出前 5 分钟内预挂、且加仓开出时仍挂着的也算这次加仓；主力开仓前预挂的仍归主力；加仓先平掉之后挂出的回到主力；反过来主力先平、加仓还开着时仍接在加仓后面；全部平完之后才挂出的，与收尾前一刻挂出的落在同一行）。这只是 Legs 表与导出 PNG 的<strong>展示位置</strong>，初始暴露、预期最大亏损等风险指标仍以主力为锚。对冲触发后的反向委托放在对应的对冲 leg；镜像止盈行则显示止盈挂单时间与触发时间，避免把主力委托误记到镜像腿。</li>
