          ...
          <section id="s4" className="scroll-mt-20">
            ...
            <section id="s4-3" className="scroll-mt-20">
              ...
              <ul className="list-disc pl-6 text-[14px] text-foreground/90 space-y-1">
                ...
                <li><strong>Legs 列表每条腿都标明开仓/平仓时间、开仓价/平仓价、仓位与状态</strong>；还没平仓的腿，平仓时间与平仓价显示「—」。平仓价右侧的<strong>「涨跌幅」</strong>是开仓价到平仓价的价格变化 =（平仓价 − 开仓价）÷ 开仓价——标的本身的涨跌，<strong>不是这条腿的盈亏</strong>：不按方向翻转，空单为负才是赚；它与左边两格用同一对价（含 K 线平仓价校正），未平仓显示「—」；主力阶段子行按各自起止价各算各的，合计行留空。PNG 导出同列。「委托」列按真实业务归属呈现：委托先按挂出时刻归到当时开着的那笔主力；<strong>加仓之后挂出的委托放在加仓那一行的后面，再加仓就放到最新那次加仓的行后面</strong>（按开仓时刻排，不看「加仓1 / 加仓2」的编号；加仓开出前 5 分钟内预挂、且加仓开出时仍挂着的也算这次加仓；主力开仓前预挂的仍归主力；加仓先平掉之后挂出的回到主力；反过来主力先平、加仓还开着时仍接在加仓后面；全部平完之后才挂出的，与收尾前一刻挂出的落在同一行）。这只是 Legs 表与导出 PNG 的<strong>展示位置</strong>，初始暴露、预期最大亏损等风险指标仍以主力为锚。对冲触发后的反向委托放在对应的对冲 leg；镜像止盈行则显示止盈挂单时间与触发时间，避免把主力委托误记到镜像腿。</li>
