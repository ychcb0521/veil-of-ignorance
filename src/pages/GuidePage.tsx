          ...
          <section id="s4" className="scroll-mt-20">
            ...
            <section id="s4-3" className="scroll-mt-20">
              ...
              <ul className="list-disc pl-6 text-[14px] text-foreground/90 space-y-1">
                ...
                <li><strong>下载选中</strong>打开设置：勾选要画进图里的<strong>战役原数据、盈亏概览、操作日情绪日记、K 线盘面、完整 Legs 列表</strong>（至少一项；情绪日记是私人记录，分享前可以取消，没有日记的战役不画这一块）；K 线周期默认<strong>自动</strong>（按每场战役自身的时长各自选，与详情页首屏一致；详情页里已保存的反事实分支会把首屏视窗撑宽、周期随之变粗，批量盘面只画战役本身，不计入分支，这类战役的自动周期可能比详情页首屏细），也可统一指定 1 分钟、5 分钟、15 分钟或 1 小时；K 线盘面与盈亏概览都不画时周期不起作用，这一组调暗停用，进度区也不写周期。<strong>指定周期是下限</strong>：盘面画的是战役前后各一倍跨度的三倍视窗，某场按所选周期超过 1000 根（每根不到 1.4 像素）时才放宽到放得下的周期，这一行在队列里标出实际周期，图里盘面标题写的也是实际周期；较短的战役按所选周期照画。<strong>盘面视窗</strong>可选 1.1x、2x、3x、5x、11x、21x、31x、41x、51x（与详情页 K 线上方那一排倍数按钮同一组），<strong>默认 1.1 倍</strong>：盘面显示战役时长的几倍、战役居中，1.1 倍几乎只放战役本身，倍数越大前后行情越多；不沿用详情页里手动拖动的视窗，没画 K 线盘面时这一项停用。</li>
              ...
              <ul className="list-disc pl-6 text-[14px] text-foreground/90 space-y-1">
                ...
                <li>反事实盘面以原始战役的 K 线、真实事件标记、竖线、区间线和委托空单为只读背景，再叠加紫色虚拟轨迹；不会为了显示反事实而丢掉原始盘面信息。<strong>反事实盘面与原始盘面同高</strong>：原始盘面按可视区实测的高度拉满，反事实盘面用同一个高度，随窗口一起变。它与原始盘面共用完整的 51 倍数据范围和快捷档位，但默认从 <strong>1.1 倍</strong>开始，在完整战役内容外保留少量上下文。</li>
