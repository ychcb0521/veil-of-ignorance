...
import {
  ArrowLeft, BookOpen, List, Clock, BarChart3, BookMarked,
  Tag as TagIcon, ScrollText, Sparkles, GitBranch, Target,
} from 'lucide-react';
...
function FlowNode({ children, accent }: { children: React.ReactNode; accent?: boolean }) {
  return (
    <div className={`bg-card border rounded p-3 text-[13px] text-center max-w-[420px] mx-auto ${
        ...
        <main className="space-y-12 min-w-0">
          ...
          <section id="s1" className="scroll-mt-20">
            ...
            <div className="space-y-3">
              <P>
                无知之幕（Veil of Ignorance）是一个加密货币合约交易的"时光机模拟器"。它的存在是为了让你在不损失真实资金的前提下，把交易能力训练到足以面对真实市场的程度。
              </P>
              <P>
                它不是一个"游戏"，也不是一个"沙盒"。它是一台训练机器——目标是让你每一次错误都成为可被分析、可被消除的输入信号。
              </P>
              <P><strong>数据来源</strong>：完全使用币安 USDT 永续合约真实历史数据。</P>
              <P>
                <strong>"无知之幕"的字面含义</strong>：在回放过程中，你永远看不到未来——K 线按真实时间 1:1 流速推进，不会暴露你尚未抵达的数据。
              </P>
            </div>
          </section>
          ...
          <section id="s2" className="scroll-mt-20">
            ...
            <div className="space-y-3">
              <P>理念分两层。</P>
              <P>
                <strong>第一层 · 时光机</strong>：你可以选择历史上任意时间点，加载该时刻的真实币安 K 线数据，以 1x-100x 倍速重放行情，并进行模拟下单。这让"过去 5 年的市场"变成你的训练场。
              </P>
              <P>
                <strong>第二层 · 错题集闭环</strong>：交易能力的提升不在交易本身，而在复盘。本系统内置一套完整的负反馈控制系统——双时点记录 + 模式聚类 + 反事实回放 + 规则回写——让你的每个错误都不只是事件，而是可被分析、归类、并最终被消除的系统漏洞。
              </P>
              <Highlight>
                "一次错误是事件，三次同模式是系统。错题集要让你的眼睛在物理上看到这种系统性。"
              </Highlight>
            </div>
          </section>
          ...
          <section id="s3" className="scroll-mt-20">
            ...
            <section id="s3-1" className="scroll-mt-20">
              <SubTitle><Clock className="inline w-4 h-4 mr-1" />3.1 时光机控制条（TIME MACHINE）</SubTitle>
            ...
            <section id="s3-2" className="scroll-mt-20">
              <SubTitle><BarChart3 className="inline w-4 h-4 mr-1" />3.2 K 线主图</SubTitle>
              <div className="space-y-2">
            ...
            <section id="s3-4" className="scroll-mt-20">
              ...
              <div className="space-y-2">
                ...
                <ul className="list-disc pl-6 text-[14px] text-foreground/90 space-y-1">
                  ...
                  <li><strong>每次下单前会强制弹出"开仓快照"——这是错题集系统的事前点</strong></li>
                </ul>
                <Highlight>
                  "开仓快照不是负担，是这个系统区别于其他模拟器的关键。它强制把你的判断、心态、风险认识固化为不可篡改的记录。"
                </Highlight>
              </div>
            </section>
          ...
          <section id="s4" className="scroll-mt-20 bg-accent/30 border border-border rounded-lg p-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-6">
              <section id="s4-1" className="scroll-mt-20 bg-card border border-border rounded p-4">
                ...
                <P className="mt-2">不是按时间排序。<strong>默认按错误模式聚类</strong>——这是和其他交易日记的根本区别。</P>
              ...
              <section id="s4-2" className="scroll-mt-20 bg-card border border-border rounded p-4">
                ...
                <P className="mt-2">包含 7 张卡片，但你只需要关心一张：<strong>规则有效性追踪</strong>。</P>
                ...
                <P className="mt-2 text-[12px] text-muted-foreground">其他 6 张卡片：错误模式趋势、alpha 时段识别、心态-收益散点、事后合理化预警、未评价积压、元元监控。</P>
              </section>
              ...
              <section id="s4-3" className="scroll-mt-20 bg-card border border-border rounded p-4">
                ...
                <ul className="list-disc pl-6 text-[13px] text-foreground/90 space-y-1 mt-1">
                  <li><strong>自动生成</strong>：同一 pattern 30 天 ≥3 次且亏损时，系统强制弹窗要求你写</li>
              ...
              <section id="s4-4" className="scroll-mt-20 bg-card border border-border rounded p-4">
                ...
                <P className="mt-2"><strong>第一层</strong>：6 大类（固定，不可改）</P>
                <ul className="list-disc pl-6 text-[13px] text-foreground/90 space-y-1 mt-1">
                  <li>入场理由错</li>
                  <li>对冲/止损错</li>
                  <li>出场理由错</li>
                ...
                <P className="mt-2"><strong>第二层</strong>：用户自定义 pattern。每个 pattern 必须配可操作定义（≥10 字符）——不能写"心态不好"，要写"交易前心态自评 ≤2 分"。</P>
              </section>
              ...
              <section id="s4-5" className="scroll-mt-20 bg-card border border-border rounded p-4 lg:col-span-2">
                ...
                <ul className="list-disc pl-6 text-[13px] text-foreground/90 space-y-1 mt-2">
                  <li><strong>通道 ①</strong>：盘面（K 线回放 + 入场/出场标记 + 止损止盈线 + 三条垂直时间线锚定决策/出场/当前）</li>
                  <li><strong>通道 ②</strong>：决策（你当时写的开仓理由、止损止盈、checklist 状态）</li>
                  <li><strong>通道 ③</strong>：状态（心态自评、心态触发原因、历史对照）</li>
                  <li><strong>通道 ④</strong>：风险认识与管理（你当时怎么想风险，事后对照实际）</li>
                  <li><strong>通道 ⑤</strong>：反事实（六步深度分析 + 反事实回放）</li>
                </ul>
              ...
              <section id="s4-6" className="scroll-mt-20 bg-card border border-border rounded p-4 lg:col-span-2">
                ...
                <P className="mt-2"><strong>六步</strong>：</P>
                <ol className="list-decimal pl-6 text-[13px] text-foreground/90 space-y-1 mt-1">
                  ...
                  <li><strong>错误类型</strong> - 一句话归纳错误的类别</li>
              ...
              <section id="s4-7" className="scroll-mt-20 bg-card border border-border rounded p-4 lg:col-span-2">
                ...
                <P className="mt-2"><strong>操作</strong>：</P>
                <ul className="list-disc pl-6 text-[13px] text-foreground/90 space-y-1 mt-1">
                ...
                <P className="mt-2">每笔 journal 最多保存 10 条分支（如"止损更宽 / 仓位减半 / 不开仓"），可分别对比。</P>
              </section>
            </div>
          </section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                ...
                <div key={d.title} className="bg-card border border-border rounded p-4 flex flex-col">
                  <div className="text-[14px] font-medium text-foreground mb-3 pb-2 border-b border-border">{d.title}</div>
