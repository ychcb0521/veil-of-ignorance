          ...
          <section id="s4" className="scroll-mt-20">
            ...
            <section id="s4-3" className="scroll-mt-20">
              ...
              <ul className="list-disc pl-6 text-[14px] text-foreground/90 space-y-1">
                ...
                <li><strong>几何期望默认看对数分布。</strong>正值按 ln(Gᵢ) 排布，刻度和提示框仍显示原始倍数：<strong>0.5 → 1 → 2 等距</strong>，本金减半与翻倍可用同一尺度比较，小于 1 的点不会被极端盈利挤成一列。竖线画在 <strong>1.00</strong>（盈亏分界），点按对数等宽分档堆叠，密度曲线也在对数空间计算，并换算成每档期望场数。<strong>Gᵢ = 0 单独显示在「本金归零」栏</strong>，不取对数、不纳入密度曲线，但仍计入样本总数、胜率和摘要统计。正值窗口在对数空间稳健取值，超出窗口的点贴边标记并计数；没有 −1R 止损墙或 +10R 封顶。原始指标计算不变，不能仅凭分布偏斜判断风控是否合格。用「时序 | 分布」切回时序。</li>
              ...
              <P>原始与反事实 Legs 在「平仓价」右侧都有<strong>操作方式</strong>：上行开仓、下行最后一笔平仓，以「手动（开）」「自动（平）」等形式对齐显示，括号弱化、手动适度突出。主力开仓（含再入场主力）按本系统约定显示手动，不扩大到加仓和镜像腿；其他腿仍按真实记录区分手动立即执行与预设委托自动成交，市价成交或历史回填本身不能证明是手动。反事实修改哪一侧的成交参数，那一侧不再冒用真实成交方式；新增模拟腿、缺少证据的旧记录显示「未记录」。</P>
