...
P 不再一把手填，而是拆成三个更可回答的问题：<strong>「这个结构还活得下去吗（P₁）」</strong>、<strong>「假如它活着，价格能走到 T 吗（P₂）」</strong>、<strong>「假如它死了，价格还是走到了 T 吗（P₃）」</strong>。P₂ 与 P₃ 都必须按<strong>条件概率</strong>来估——先假定结构的死活，再问突破的把握，否则会把结构风险重复计价。
          ...
          <section id="s4" className="scroll-mt-20">
            ...
            <section id="s4-3" className="scroll-mt-20">
              ...
              <P>盈亏比、预期回撤、机会质量、算术期望、几何期望、重要性、镜像止盈、DSI 贡献、USI 贡献都各自配有一张散点图。在排序行上<strong>双击或右键</strong>对应指标打开公式浮层，浮层底部的「查看散点图」即可展开；再点一次「收起散点图」关闭。除盈亏比分布图外，横轴是按<strong>客观操作时间</strong>排列的战役序号，与列表当前排序无关——改排序不会让点位移动。</P>
              <ul className="list-disc pl-6 text-[14px] text-foreground/90 space-y-1">
                ...
                <li><strong>盈亏比多一张分布图。</strong>在赔率图面板右上角用「时序 | 分布」切换：分布图横轴是盈亏比 b 本身（单位 R，线性刻度），纵轴是落在该 b 附近的战役数量——点按档从底线向上堆叠（每 1R 等分成若干档、每档至少 14px 宽，−1R 与 0 恰好是档边界），堆得越高出现得越多，不考虑时间先后。琥珀色 −1R 虚线是止损墙，灰色 0 线是盈亏平衡，灰色曲线是核密度估计换算成「每档期望场数」的平滑轮廓，与柱共用同一条场数轴；显示区间取 p2–p98 并封顶在 +10R，超出右缘的极端盈利贴边画成三角并在脚注计数，−1R 左侧的亏损照常落在墙外。</li>
