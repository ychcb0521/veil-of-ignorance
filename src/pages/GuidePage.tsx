...
P 不再一把手填，而是拆成三个更可回答的问题：<strong>「这个结构还活得下去吗（P₁）」</strong>、<strong>「假如它活着，价格能走到 T 吗（P₂）」</strong>、<strong>「假如它死了，价格还是走到了 T 吗（P₃）」</strong>。P₂ 与 P₃ 都必须按<strong>条件概率</strong>来估——先假定结构的死活，再问突破的把握，否则会把结构风险重复计价。
            ...
            <section id="s3-1c" className="scroll-mt-20">
              ...
              <RedHighlight>
                ...
                <strong>撤掉带计划的限价 / 条件单</strong>（包括成交时保证金不足被撤），计划仍在保鲜期、又没有更新的计划、这个标的也仍持有同方向仓位时会放回去，紧接着追价的同向单照样带上（停止回放先平仓再撤单，计划不会漏到下一场）；
                ...
                「<strong>按上限下单</strong>」按钮把整张的上限连同下单方式直接预填进下单面板（U 本位的币数按数量精度向下取整；面板的结算方式与计划不同就先切过去），
          ...
          <section id="s4" className="scroll-mt-20">
            ...
            <section id="s4-3" className="scroll-mt-20">
              ...
              <P>盈亏比、预期回撤、机会质量、算术期望、几何期望、重要性、镜像止盈、DSI 贡献、USI 贡献都各自配有一张散点图。在排序行上<strong>双击或右键</strong>对应指标打开公式浮层，浮层底部的「查看散点图」即可展开；再点一次「收起散点图」关闭。<strong>盈亏比与几何期望默认展开的是分布图、镜像止盈默认展开的是柱状图</strong>，其余指标以及这三者切回「时序」后，横轴都是按<strong>客观操作时间</strong>排列的战役序号，与列表当前排序无关——改排序不会让点位移动。</P>
              <ul className="list-disc pl-6 text-[14px] text-foreground/90 space-y-1">
                ...
                <li><strong>几何期望默认看分布。</strong>要判断的是这套打法的<strong>资本增长偏不偏</strong>：左边贴着 1.00 堆着一大群小亏小赚、右边拖出一条长尾，正是「亏损受控、盈利开放」该有的形状；若左尾反而更长，说明亏的时候没收住。横轴是 Gᵢ 本身，竖线画在 <strong>1.00</strong>（盈亏分界）；盈亏比专属的 −1R 止损墙与「右尾 &gt;+5R」计数在这里没有意义，都不出现。窗口按 p2–p98 取，超出的极端值贴边画成三角并在脚注计数。用面板右上角的「时序 | 分布」切回时序。</li>
              ...
              <ul className="list-disc pl-6 text-[14px] text-foreground/90 space-y-1">
                ...
                <li><strong>「Δb」列与主力阶段拆解。</strong>Δb = 该腿盈亏 ÷ 战役初始最大预期亏损 L——这条腿把整场盈亏比 b <strong>推高或拉低了多少个单位</strong>，Σ(各腿 Δb) 恰为战役已实现 b，「b 是如何被增加、如何被削减」由此逐腿可见。更进一步，<strong>主力不是铁板一块：每一次滚动对冲的结束，意味着主力一个阶段的完成</strong>。主力行下方有浅色的<strong>阶段子行</strong>，<strong>默认折叠</strong>：主力的角色标签右边有一个小开关——朝右的箭头加阶段数（如「› 5」，悬停显示「展开 5 个阶段」），点一下展开、箭头转向下，再点收起；各行的开关落在同一条竖线上，每条主力各自记着开合，刷新后回到折叠。阶段子行按各次对冲的平仓时刻把主力切段（边界价即对冲平仓价——同一时刻的市价），每段给出时间区间、起止价、分摊盈亏、贡献率与 Δb。分摊按价差权重进行、<strong>各段之和严格等于主力整腿盈亏</strong>（Σ阶段贡献 = 主力贡献、Σ阶段 Δb = 主力 Δb，三重守恒），绝不臆造数字。一场「整腿 +3.2 万」的主力可能拆出「对冲结束前 +4.0 万、之后回吐 −0.8 万」——哪一段决策在挣钱、哪一段在回吐，据此可见。贴着主力平仓结束的对冲不切段；只有一段时不显示子行，角色标签旁也没有阶段开关。PNG 导出同源同列；PNG 导出不跟着折叠，始终画出全部阶段子行。</li>
                ...
                <li><strong>Legs 列表每条腿都标明开仓/平仓时间、开仓价/平仓价、仓位与状态</strong>；还没平仓的腿，平仓时间与平仓价显示「—」。<strong>第一列只有「角色」</strong>：一枚角色标签，不印腿的序号，也不挂「回填」标签（历史回填的腿，鼠标停在角色标签上可见说明）；横向滚动时这一列冻结在左缘，滚出去之后右缘才出现一道分隔线。状态不单独占一列，也不再写成文字标签，而是画在角色标签上：已平仓是常态，标签照常是实心淡底；<strong>「挂单中」</strong>（对冲 / 镜像腿还没有成交或平仓记录）的标签画成同色虚线的空心标签，<strong>「进行中」</strong>（还没平仓）的标签文字后面多一枚同色实心小圆点；两种都能悬停看到说明，读屏也会念出状态。没有角色的腿是一枚写着「—」的灰色标签（悬停说明它还没有归类），状态的画法相同。PNG 导出的第一列同样只有角色标签，挂单中空心、进行中带圆点。平仓价右侧的<strong>「涨跌幅」</strong>按持仓方向计（多单 =（平仓价 − 开仓价）÷ 开仓价，空单 =（开仓价 − 平仓价）÷ 开仓价），正数即这条腿在价格上占优、按所示的这一对开平价看与盈亏同号；与左边两格用同一对价（含 K 线平仓价校正），未平仓显示「—」；一个仓位分几刀平掉时平仓价取最后一刀、盈亏是各刀合计，两者符号可能不同；主力阶段子行按主力方向、各自起止价各算各的，合计行留空。PNG 导出同列。「币量 / 仓位」右侧是<strong>「多单占比」</strong>一列，<strong>只算多单</strong>：按这条腿实际的持仓方向分组（与「涨跌幅」同一个方向），多单各腿只和多单比——对冲通常是空单，主空战役里的对冲则是多单，分组跟方向走、不跟角色走。列头挂一枚<strong>「多」（绿）标签</strong>；<strong>空单的行这一列留空</strong>，空单也不进它的分母——空单不单独算占比，只在合计行给出总量；百分数本身是中性色，悬停可见「多单合计里的占比」。上行是这条腿的币量占多单币量合计的百分比、下行是名义仓位占多单名义仓位合计的百分比，两个分母各算各的，用的是「币量 / 仓位」列每行背后的同一组数，按未舍入的原值相加（各行与合计行各自取两位小数，把各行印出来的数手工相加，末位可能与合计行对不上）；状态为「挂单中」的对冲 / 镜像腿（还没有成交或平仓记录）不计入合计，是多单时这一列两行都显示「—」；缺开仓价的多单上行显示「—」、不进多单的币量合计；各行分别取一位小数，逐行相加不一定恰好是 100.0%（腿越多、舍入差可能越大，如六条等额的多单各印 16.7%，加起来是 100.2%）；阶段子行这一列留空。<strong>点击列头排序</strong>：点一下按多单占比降序（占比大的在上），再点一下升序，第三下回到默认顺序（降序 → 升序 → 默认顺序）；按上行的币量占比排，缺币量时按下行的名义仓位占比；这一列没有数的行（空单、挂单中的腿、没有仓位数据的腿）不论升降序都留在最下面，彼此保持原来的先后，占比相同的也保持原来的先后；主力的阶段子行跟着主力走，合计行始终在最后；排序后表格回到最上面（横向位置不变），排在最前的行直接看得见；排序不保存，刷新或重新打开这场战役后回到默认顺序。合计行的「币量 / 仓位」格按方向各写一组 Σ（以同样的「多 / 空」标签开头，上行 Σ币量、下行 Σ名义仓位，挂单中的腿不计入）：<strong>「多」那组是「多单占比」的分母</strong>，「多单占比」的合计格写多单的「100.0%」，与这组落在同一行；<strong>「空」那组是空单各腿的合计</strong>，看对冲一共开了多大，不作任何占比的分母；没有计入腿的方向不列（例如唯一的对冲还挂单中时只剩多单一组；多单都还挂单中时「多单占比」的合计格留空）；一条腿都不计入时这两格都显示「—」（某方向计入的腿都缺开仓价时，该方向只有上行是「—」）。PNG 导出同样只有「多单占比」一列，留空与合计行对齐的规则相同；PNG 导出不跟着排序，始终按默认顺序画出全部腿。「委托」列按真实业务归属呈现：委托先按挂出时刻归到当时开着的那笔主力；<strong>加仓之后挂出的委托放在加仓那一行的后面，再加仓就放到最新那次加仓的行后面</strong>（按开仓时刻排，不看「加仓1 / 加仓2」的编号；加仓开出前 5 分钟内预挂、且加仓开出时仍挂着的也算这次加仓；主力开仓前预挂的仍归主力；加仓先平掉之后挂出的回到主力；反过来主力先平、加仓还开着时仍接在加仓后面；全部平完之后才挂出的，与收尾前一刻挂出的落在同一行）。这只是 Legs 表与导出 PNG 的<strong>展示位置</strong>，初始暴露、预期最大亏损等风险指标仍以主力为锚。对冲触发后的反向委托放在对应的对冲 leg；镜像止盈行则显示止盈挂单时间与触发时间，避免把主力委托误记到镜像腿。</li>
                <li><strong>「加仓校验」列</strong>（紧跟「币量 / 仓位」与「多单占比」之后）只在加仓行打标：每笔加仓只按<strong>它自己那一刻</strong>的客观状态判定（马尔可夫式）：当时仍持有的全部腿按开仓价入账——更早的加仓在当时止损线上的浮盈加进垫子、浮亏扣减垫子——加上当时挂着的止损线与当时已落袋的 G；不继承上一笔加仓的对错，上一笔判 ✗ 不会让这一笔自动判 ✗，上一笔的浮亏尚未兑现、而当下已出现可加仓的优势时，照当下判。以加仓那一刻挂着（或加仓后 5 分钟内补挂）、在亏损侧离加仓价最近的反向委托价为止损线 S₁（加仓计算器比对盘口线用同一规则），<strong>旧仓浮盈垫 X₁(S₁ − S̄) + 已落袋 G ≥ 新加仓最大预期亏损 X₂(S₂ − S₁)</strong>（主空符号翻转，不计手续费；币本位 G 按 S₁ 折成 U）即合规——X₁ 只算加仓那一刻还拿着的币（加仓前已减掉、已止盈的不算）；G 的正向来源只认成交记录上退出方式为止盈1 的那几刀（镜像止盈），普通减仓、手动或止损平仓的盈利不混入——哪怕合并仓位按成交占比分到了镜像腿上；先前已经实现的亏损（含强平）则从 G 中扣掉，G 可以是负数；再入场之前那一轮也不算。与加仓计算器同一个 G。合规时显示一枚几乎隐形的淡灰对号；抹不平就是<strong>红色放大的叉</strong>，下方直接写出 Plan B 的<strong>正确加仓上限</strong>：X₂ 是币量，同时给出按 S₂ 折算的 U 名义仓位，两个数是同一仓位的两种单位，不可相加。<strong>点击红叉</strong>会展开计算框，逐步列出 Y₁、G、可用覆盖额、每币风险、正确币量上限、U 名义仓位以及实际超出量；读不到 S₁ 等无法判断时显示淡灰「—」。PNG 导出也会写明币量上限与 U 折算额。<strong>S₂ 一律按成交价判</strong>——市价单含引擎滑点（0.01% + 名义 ÷ 50 亿）的那个价，不是下单前看到的现价；真正的风险就在成交价上，判定不为计算器让步。成交记录带着加仓计算器当时的计划时，计算框与 PNG 会并排写出「计算时 现价 …，预计成交 …，上限 … 币；实际成交 …，上限 … 币」（括号里是相对这张单下单价的滑点；「现价」只用于市价计划——限价计划的参考价是手填的限价，写「计算时 限价 …，挂单价 …」，条件委托计划写「计算时 触发价 …」），再说清超出从哪来：
              ...
              <ul className="list-disc pl-6 text-[14px] text-foreground/90 space-y-1">
                ...
                <li>已保存分支逐行列出名字、改了什么、保存时间、分支 P&L 与相对实际；点一行就在下方展开同样的一行：右栏「反事实盈亏概览 · 名字」是同一套指标，左栏「相对原始的变化情况」另写分支类型（推演分支带 SOP 分数）与保存时刻，可以在那里「删除」，也可以「载入到 Legs 副本」把那条分支的腿载回编辑器继续调整、再次运行。载入或手改的腿只在战役的 Legs、成交记录或平仓价校正变化时才会被重置回原始腿，点「保存备注」不会。每场战役最多保留 <strong>20</strong> 条分支，超出时最早的会被自动清理。</li>
                ...
                <li>下方「Legs 副本 · 手动反事实」是<strong>可编辑</strong>的：盘面竖线可拖动、表格逐格可改，改完直接「一键运行」；「还原 Legs」回到原始战役的腿。</li>
              </ul>
              <P><strong>偏离代价明细（手动调整 vs 原始）</strong>把你手动调整后的 Legs 与原始战役逐腿对比，把原始错误折算成钱：</P>
              <ul className="list-disc pl-6 text-[14px] text-foreground/90 space-y-1">
              ...
              <table className="w-full text-[11px] my-3 border border-border rounded overflow-hidden">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td className="px-3 py-2 border-t border-border">同标的同方向的多笔仓位会并成一张卡。卡上的<strong>强平价格（最先）</strong>写的是这组里<strong>最先被强平</strong>的那一笔的价格——多单取各笔中最高的，空单取最低的。<strong>「+」调整保证金在合并卡上一直可用，合并几笔都一样</strong>：追加按各笔名义等比摊分，减少按各笔<strong>还能减多少</strong>等比摊分，减到各自的开仓保证金为止。整组一次写完，按仓位 id 定位。分组只按标的+方向、<strong>不含保证金模式</strong>，所以全仓腿会并进同一张卡——此时按钮照常可用，<strong>只对其中的逐仓腿生效</strong>，并在弹窗里写明有几笔全仓未计入。整张卡都是全仓时按钮置灰保留并给出原因：全仓共用一个保证金池，单仓位追加在机制上不存在。</td>
