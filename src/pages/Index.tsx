...
const Index = () => {
  ...
  const createTriggeredConditionalPosition = useCallback((symbol: string, order: PendingOrder, triggerPrice: number, openTime: number) => {
    ...
    setTradeHistory(prev => [...prev, {
      id: crypto.randomUUID(), symbol, side: order.side,
      type: order.type as OrderType, action: 'OPEN',
      entryPrice, exitPrice: 0,
      quantity: order.quantity, leverage: order.leverage,
      pnl: 0, fee, slippage: 0,
  ...
  useEffect(() => {
    ...
    const candidate = Number(
      refPrice > 0 && !isCrossSymbolPollution
        ? refPrice
        : visPrice
    );
      ...
      setOrdersMap(prev => {
        ...
        for (const order of orders) {
          ...
          switch (order.type) {
            ...
            case 'LIMIT_TP_SL': {
              ...
              if (triggerHit) {
                ...
                else { convertToLimit = true; updatedOrder = { ...order, type: 'LIMIT', status: 'ACTIVE' } as PendingOrder; }
              }
              break;
            }
            case 'TRAILING_STOP': {
          ...
          if (triggered) {
            ...
            setTradeHistory(prev => [...prev, {
              id: crypto.randomUUID(), symbol: activeSymbol, side: matchedOrder.side,
              type: matchedOrder.type as OrderType, action: 'OPEN',
              entryPrice: actualFillPrice, exitPrice: 0,
          ...
          if (order.twapNextExecTime && now >= order.twapNextExecTime) {
            ...
            if (filledSoFar + sliceQty <= totalQty + 0.0001 && now < endTime) {
              ...
              setTradeHistory(prev => [...prev, {
                id: crypto.randomUUID(), symbol, side: order.side,
                type: order.type as OrderType, action: 'OPEN',
                entryPrice: slippedPrice, exitPrice: 0,
                quantity: sliceQty, leverage: order.leverage,
                pnl: 0, fee, slippage: slippageAmt,
                openTime: now, closeTime: 0,
              }]);
              changed = true;
