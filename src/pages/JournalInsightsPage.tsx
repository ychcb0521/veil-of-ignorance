...
export default function JournalInsightsPage() {
  ...
  const inRange = (j: TradeJournal) => Date.now() - new Date(j.pre_simulated_time).getTime() <= range * DAY;
  ...
  const stats = useMemo(() => {
    ...
    const alphaHours = timeDist
      .filter((b) => b.count >= 3 && b.avg_pnl > 0)
    ...
    const ruleEffect = data.rules
      ...
      .map((r) => {
        ...
        const before = data.assignments.filter(
          (a) =>
            a.pattern_id === r.source_pattern_id &&
            new Date(a.created_at).getTime() < since &&
            new Date(a.created_at).getTime() >= since - range * DAY,
        ).length;
        const after = data.assignments.filter(
          (a) => a.pattern_id === r.source_pattern_id && new Date(a.created_at).getTime() >= since,
        ).length;
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="max-w-[1400px] mx-auto px-6 py-4 space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          ...
          <StatCard
            label="胜率"
            value={`${(stats.outcome.win_rate * 100).toFixed(0)}%`}
            accent={stats.outcome.win_rate >= 0.5 ? "#0ECB81" : "#F6465D"}
          />
          <StatCard
            label="期望 R"
            value={stats.outcome.expectancy.toFixed(2)}
            accent={stats.outcome.expectancy >= 0 ? "#0ECB81" : "#F6465D"}
          />
          ...
          {stats.trend.length === 0 ? (
            ...
            <table className="w-full text-[11px]">
              <thead className="text-muted-foreground bg-background">
                <tr>
                  <th className="text-left px-3 py-1.5">模式</th>
                  <th className="text-right px-3">本周期</th>
                  <th className="text-right px-3">上周期</th>
                  <th className="text-right px-3">Δ</th>
                  <th className="text-right px-3 pr-3">avg P&L</th>
                </tr>
              </thead>
              <tbody className="font-mono">
                {stats.trend.slice(0, 12).map((t) => {
                  const deltaColor =
                    t.delta > 0 ? "text-[#F6465D]" : t.delta < 0 ? "text-[#0ECB81]" : "text-muted-foreground";
                  return (
                    <tr key={t.pattern.id} className="border-t border-border">
                      ...
                      <td className={`text-right px-3 ${deltaColor}`}>
                        {t.delta > 0 ? "+" : ""}
                        {t.delta}
                      </td>
          ...
          <section className="border border-border rounded bg-card p-3">
            ...
            {stats.mixedRBasis && (
              <div className="mt-2 text-[10px] text-muted-foreground">
                R 倍数计算基于"本次预设最大亏损"。历史 journal（使用"预设止损价"计算）的 R 数据仍可用，但口径与新
                journal 略有差异。
              </div>
            )}
          </section>
              ...
              {stats.mentalDist.map((m) => (
                <div key={m.state} className="flex items-center gap-2 text-[11px] font-mono">
                  ...
                  <div className="flex-1 bg-background h-2 rounded overflow-hidden relative">
                    <div
                      className={`h-full ${m.avg_pnl >= 0 ? "bg-[#0ECB81]" : "bg-[#F6465D]"}`}
        ...
        <div className="grid md:grid-cols-2 gap-3">
            ...
            <div className="space-y-2">
              <div className="rounded border border-border bg-background p-2">
                ...
                <div className="mt-1 text-[10px] text-muted-foreground font-mono">
                  胜率 {(stats.orderTypeStats.main.winRate * 100).toFixed(0)}% · 平均 R{" "}
                  {stats.orderTypeStats.main.avgR.toFixed(2)}
                </div>
              </div>
              <div className="rounded border border-border bg-background p-2">
                <div className="flex items-center justify-between text-[11px]">
                ...
                <div className="mt-1 text-[10px] text-muted-foreground font-mono">
                  胜率 {(stats.orderTypeStats.hedge.winRate * 100).toFixed(0)}% · 平均 R{" "}
                  {stats.orderTypeStats.hedge.avgR.toFixed(2)}
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="border border-border rounded bg-card p-3">
            ...
            <div className={`mt-2 text-[11px] ${stats.crossCount === 0 ? "text-[#0ECB81]" : "text-[#F6465D]"}`}>
              {stats.crossCount === 0
                ? "✓ 守卫上线后无任何全仓交易"
        ...
        {/* Rule effectiveness */}
        <section className="border border-border rounded bg-card">
          <div className="px-3 py-2 border-b border-border text-[12px] font-medium">
            规则有效性（生效前后该 pattern 的出现次数）
          </div>
            ...
            <table className="w-full text-[11px]">
              <thead className="text-muted-foreground bg-background">
                <tr>
                  ...
                  <th className="text-right px-3">规则前</th>
                  <th className="text-right px-3">规则后</th>
                  <th className="text-right px-3 pr-3">Δ</th>
                </tr>
              </thead>
              <tbody className="font-mono">
                {stats.ruleEffect.map((e) => {
                  const sourceLabel = e.rule.source_pattern_id ? "模式触发" : "手动";
                  const sourceColor = e.rule.source_pattern_id
                  ...
                  return (
                    <tr key={e.rule.id} className="border-t border-border">
                      ...
                      <td className="px-3">
                        <span className={`inline-block rounded px-1.5 py-0.5 text-[10px] ${sourceColor}`}>
                          {sourceLabel}
                        </span>
                      </td>
                      ...
                      <td
                        className={`text-right px-3 pr-3 ${e.delta < 0 ? "text-[#0ECB81]" : e.delta > 0 ? "text-[#F6465D]" : "text-muted-foreground"}`}
                      >
                        {e.delta > 0 ? "+" : ""}
                        {e.delta}
                      </td>
...
function StatCard({ label, value, accent, sub }: { label: string; value: string; accent?: string; sub?: string }) {
  return (
    <div className="border border-border rounded bg-card p-3">
      ...
      <div className="text-[22px] font-mono mt-1" style={{ color: accent }}>
        {value}
      </div>
