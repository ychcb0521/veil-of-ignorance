...
const DARK_CHART_STYLES = {
  ...
  crosshair: {
    ...
    horizontal: {
      ...
      text: { show: true, color: "#FFF", borderColor: "#F0B90B", backgroundColor: "#363A45" },
    },
    vertical: {
      show: true,
      line: { show: true, color: "#F0B90B33", style: LineType.Dashed },
...
const LIGHT_CHART_STYLES = {
  ...
  crosshair: {
    ...
    horizontal: {
      show: true,
      line: { show: true, color: "#B7BDC633", style: LineType.Dashed },
...
export function TradeInsightsPanel({ open, onClose, tradeHistory, initialSymbol, onJumpToTime }: Props) {
  ...
  // Create customized styles on the fly so we don't have static global objects that can't use dynamic formatters
  const currentChartStyles = useMemo(() => {
    const base = theme === "light" ? LIGHT_CHART_STYLES : DARK_CHART_STYLES;
    return {
      ...base,
      candle: {
        ...base.candle,
        tooltip: {
          ...
          custom: (data: any) => {
            ...
            return [
              ...
              { title: "开 ", value: current.open.toFixed(4) },
              { title: "高 ", value: current.high.toFixed(4) },
              { title: "低 ", value: current.low.toFixed(4) },
              { title: "收 ", value: current.close.toFixed(4) },
  ...
  }, [open, currentChartStyles]);

  // Theme sync
  useEffect(() => {
    chartRef.current?.setStyles(currentChartStyles);
  }, [currentChartStyles]);
  ...
  useEffect(() => {
    ...
    fetchKlines(selectedCoin, interval, effectiveStart, effectiveEnd)
      .then((klines) => {
        ...
        <div className="flex-1 flex min-h-0 overflow-hidden">
          ...
          <div className="flex-1 min-w-0 flex flex-col relative">
            ...
            {selectedTooltip && (
              ...
              >
                <div
                  className="rounded-lg border border-primary/40 shadow-xl p-3 min-w-[200px]"
                  style={{ background: "hsl(var(--popover))" }}
                >
                  ...
                  <div className="grid grid-cols-2 gap-x-3 gap-y-1 text-[10px] font-mono">
                    ...
                    <span className="text-foreground tabular-nums">
                      {selectedTooltip.pair.open.entryPrice.toFixed(2)}
                    </span>
                    <span className="text-muted-foreground">平仓价</span>
                    <span className="text-foreground tabular-nums">
                      {selectedTooltip.pair.close.exitPrice.toFixed(2)}
                    </span>
                    ...
                    return (
                      <div
                        ...
                        onClick={() => {
                          ...
                          setSelectedTooltip((prev) =>
                            prev && selectedPairIdx === idx ? null : { x: 300, y: 300, pair },
                          );
