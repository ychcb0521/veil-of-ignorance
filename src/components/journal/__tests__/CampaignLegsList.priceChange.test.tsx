 ...
 * 涨跌幅 =（平仓价 − 开仓价）÷ 开仓价，标的本身的涨跌——不按方向翻转，不是这条腿的盈亏。
 */
...
const renderList = (legs: TradeJournal[], legExitPriceCorrections: LegExitPriceCorrections = {}) => render(
  <MemoryRouter>
    <CampaignLegsList
      legs={legs}
      tradeRecords={[]}
...
describe('Legs 列表的「涨跌幅」列', () => {
  it('表头紧跟在平仓价之后、币量 / 仓位之前，右对齐，tooltip 说明它不是盈亏', () => {
    ...
    expect(header.getAttribute('title')).toContain('不是这条腿的盈亏');
    expect(header.getAttribute('title')).toContain('空单为负才是赚');
  });
  ...
  it('空单价格下跌：负数、红色——空单为负才是赚，不按方向翻转', () => {
    renderList([legFor({
  ...
  describe('主力阶段子行与合计行', () => {
    ...
    it('阶段子行按各自起止价各算各的，淡色行里仍按 /90 绿涨红跌', () => {
      ...
      // 对冲腿自己：空单价格从 0.05 涨到 0.052 仍是正数
