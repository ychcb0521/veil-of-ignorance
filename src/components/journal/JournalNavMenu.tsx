...
export function JournalNavMenu({ onOpenAssets, onOpenAnalytics }: Props) {
  ...
  const triggerBtn = (extraClass = '') => (
    <button
      className={`flex items-center gap-1.5 px-2 py-1 rounded text-[12px] text-muted-foreground hover:text-foreground hover:bg-[#181A20] transition-colors ${extraClass}`}
    >
      ...
      <div className="md:hidden">
        <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
          <SheetTrigger asChild>
            <button className="flex items-center gap-1.5 px-2 py-1 rounded text-[12px] text-muted-foreground hover:text-foreground hover:bg-[#181A20] transition-colors">
              <LayoutGrid className="h-3.5 w-3.5" />
              <span>复盘中心</span>
            </button>
          </SheetTrigger>
          <SheetContent
            side="right"
            className="bg-[#181A20] border-[#2B3139] text-foreground w-[260px] p-0"
          >
            <SheetHeader className="px-4 py-3 border-b border-[#2B3139]">
              <SheetTitle className="text-[13px] text-foreground text-left">复盘中心</SheetTitle>
            </SheetHeader>
            <div className="p-2">
              {items.map((it, i) => {
                if (it === 'sep') return <div key={`sep-${i}`} className="h-px my-1 bg-[#2B3139]" />;
                const Icon = it.icon;
                const active = it.isActive();
                return (
                  <button
                    ...
                    className={`w-full h-10 px-3 text-[13px] rounded hover:bg-[#2B3139] flex items-center gap-2 ${active ? 'text-[#F0B90B] bg-[#F0B90B]/5' : ''}`}
                  >
