...
import { LayoutGrid, ChevronDown, CheckCheck, Wallet, BookOpen, Gauge, ShieldCheck, Tags } from "lucide-react";
...
function useNavItems(onOpenAssets: () => void): (NavItem | "sep")[] {
...
export function JournalNavMenu({ onOpenAssets }: Props) {
  const [open, setOpen] = useState(false);
  const [sheetOpen, setSheetOpen] = useState(false);
  const items = useNavItems(onOpenAssets);
    ...
    <>
      ...
      <div className="hidden md:block">
        <DropdownMenu open={open} onOpenChange={setOpen}>
          ...
          <DropdownMenuContent align="end" className="bg-card border-border text-foreground min-w-[200px] p-1">
            {items.map((it, i) => {
      ...
      <div className="md:hidden">
        <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
          ...
          <SheetContent side="right" className="bg-card border-border text-foreground w-[260px] p-0">
            ...
            <div className="p-2">
              {items.map((it, i) => {
                ...
                return (
                  ...
                  >
                    {active ? (
                      <CheckCheck className="h-4 w-4 text-[#F0B90B]" />
                    ) : (
