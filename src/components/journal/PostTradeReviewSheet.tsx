...
import {
  ...
  listAssignmentsForJournal,
  countPatternOccurrencesLast30Days,
  listPatterns,
...
export function PostTradeReviewSheet({ isOpen, onOpenChange, journal, tradeRecord, onReviewed, onAutoPause }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        post.forEach((a) => {
          if (a.note) ns[a.pattern_id] = a.note;
        });
        setTagNotes(ns);
  ...
  const outcome: TradeOutcome =
    journal?.direction === "no_entry" ? "no_entry" : pnl > 0 ? "win" : pnl < 0 ? "loss" : "breakeven";
  const computedR = useMemo(() => {
    const ml = journal?.pre_max_loss_usdt;
    if (!ml || ml === 0) return null;
    return pnl / ml;
  }, [pnl, journal?.pre_max_loss_usdt]);
  const finalR = rMultipleOverride !== "" && !isNaN(Number(rMultipleOverride)) ? Number(rMultipleOverride) : computedR;
      ...
      <div className="px-5 py-4 space-y-4 overflow-y-auto flex-1">
        ...
        <Collapsible defaultOpen>
          ...
          <CollapsibleContent className="mt-2 space-y-1 text-[11px] font-mono text-foreground/80 pl-3 border-l border-border">
            ...
            <div>
              • {isHedge ? "对冲理由" : "入场理由"}：{journal.pre_entry_reason}
            </div>
            {journal.pre_planned_stop_loss != null ? (
              <div>
                • 预设止损/止盈：{journal.pre_planned_stop_loss} / {journal.pre_planned_take_profit ?? "—"}{" "}
            ...
            {journal.direction !== "no_entry" && (
              <div>
                • 杠杆 / 仓位模式：{journal.leverage != null ? `${journal.leverage}x` : "—"} · {positionModeLabel}
              </div>
            )}
            {checklistItemsArr.length > 0 && (
              <div>
                • Checklist：{reqChecked}/{checklistRequired.length} 必填 · {optChecked}/{checklistOptional.length} 可选
                · {journal.pre_checklist_passed ? "通过" : "未通过"}
              </div>
            )}
          </CollapsibleContent>
        </Collapsible>
        ...
        <Collapsible open={sixStepOpen} onOpenChange={setSixStepOpen}>
          <CollapsibleTrigger className="w-full bg-card border border-border rounded px-3 py-2 flex items-center gap-2 hover:bg-[#1f242c]">
            ...
            <ChevronDown
              className={`w-3.5 h-3.5 text-muted-foreground transition-transform ${sixStepOpen ? "rotate-180" : ""}`}
            />
          </CollapsibleTrigger>
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex items-center justify-between">
            <p className="text-[10px] text-muted-foreground">
              ❗ 必须可执行——不是"下次更冷静"，而是"下次开仓前必须满足 checklist 第 N 项"
            </p>
          ...
          <div className="bg-[#F6465D]/10 border border-[#F6465D]/30 rounded p-3 space-y-1">
            {hotWarnings.map((w) => (
              <div key={w.pattern.id} className="text-[12px] text-[#F6465D]">
                ...
                次后，系统会在批次 6 强制要求你写一条新规则加入 checklist。
              </div>
            ))}
          </div>
        )}
