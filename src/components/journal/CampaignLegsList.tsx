...
export function CampaignLegsList({ legs, tradeRecords }: Props) {
  const nav = useNavigate();
  const recordMap = useMemo(() => new Map(tradeRecords.map(record => [record.id, record])), [tradeRecords]);

  return (
    <div className="bg-card border border-border rounded overflow-hidden">
      <div className="grid grid-cols-[48px_120px_1fr_96px_92px_88px_72px_84px] text-[10px] text-muted-foreground bg-muted/40 py-2 px-3">
        <div>#</div>
        <div>角色</div>
        <div>时间</div>
        <div>价格</div>
      ...
      {legs.map(leg => {
        ...
        return (
          <div
            key={leg.id}
            className="grid grid-cols-[48px_120px_1fr_96px_92px_88px_72px_84px] items-center text-[11px] font-mono py-2 px-3 border-b border-border/40 hover:bg-accent"
          >
