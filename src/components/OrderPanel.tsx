...
export function OrderPanel({
  ...
  }, [pickedPrice, pickMode, pricePrecision, onPickModeChange]);
  ...
  const needsLimitPrice =
    ["LIMIT", "POST_ONLY", "LIMIT_TP_SL", "CONDITIONAL", "SCALED"].includes(orderType) ||
  ...
  return (
    <div className="flex flex-col h-full bg-card">
          ...
          {(["cross", "isolated"] as const).map((m) => (
            <button
              ...
              className={`flex-1 py-1 rounded text-xs font-medium transition-all duration-100 ease-out active:scale-[0.97] ${
                marginMode === m ? "bg-accent text-foreground" : "text-muted-foreground hover:text-foreground"
              }`}
            >
              {m === "cross" ? "全仓" : "逐仓"}
            </button>
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mb-2">
            <button
              ...
              className="w-7 h-7 rounded bg-secondary flex items-center justify-center hover:bg-accent active:scale-95 transition-all text-foreground text-sm font-bold"
            >
              −
            </button>
            <div className="relative flex-1 max-w-[80px]">
              ...
              />
              <span className="absolute right-1.5 top-1/2 -translate-y-1/2 text-xs font-bold text-muted-foreground pointer-events-none">
                x
              </span>
            </div>
            <button
        ...
        <div className="flex items-center justify-between text-[10px] pt-1">
          {onTogglePriceProtection && (
            <label className="flex items-center gap-1.5 cursor-pointer">
              <input
                type="checkbox"
                checked={priceProtection ?? true}
                onChange={() => onTogglePriceProtection()}
      ...
      <div className="px-3 pt-2 pb-1 relative" ref={dropdownRef}>
        ...
        >
          <span>
            {selectedInfo.label} <span className="text-muted-foreground ml-1">{selectedInfo.desc}</span>
          </span>
      ...
      <div className="px-3 space-y-2 flex-1 overflow-y-auto pb-2">
        ...
        {/* === Post Only note === */}
        {orderType === "POST_ONLY" && (
          <div className="text-[10px] text-primary/70 px-1">⚠ 如果会立即成交则自动撤回</div>
        )}
        ...
        {orderType === "TRAILING_STOP" && (
          <>
            ...
            {trailingExecType === "LIMIT" && (
              <InputField
                label="限价 (USDT)"
                value={trailingLimitPrice}
                onChange={setTrailingLimitPrice}
                placeholder={currentPrice.toFixed(pricePrecision)}
              />
            )}
          </>
        ...
        {orderType === "TWAP" && (
          <>
            ...
            <InputField
              label="下单间隔 (分钟)"
              value={twapInterval}
              onChange={setTwapInterval}
              placeholder="5"
              suffix="min"
            />
            <div className="text-[10px] text-muted-foreground px-1">
              将拆分为 {Math.max(1, Math.floor((parseFloat(twapDuration) || 60) / (parseFloat(twapInterval) || 5)))}{" "}
              笔子订单， 每笔{" "}
              {(
                effectiveQty /
                Math.max(1, Math.floor((parseFloat(twapDuration) || 60) / (parseFloat(twapInterval) || 5)))
              ).toFixed(4)}
            </div>
          </>
        )}
        ...
        {orderType === "SCALED" && (
          <>
            ...
            <InputField
              ...
              onChange={setScaledStartPrice}
              placeholder={currentPrice.toFixed(pricePrecision)}
            />
            <InputField
              label="终点价 (USDT)"
              value={scaledEndPrice}
              onChange={setScaledEndPrice}
              placeholder={currentPrice.toFixed(pricePrecision)}
            />
            <div className="text-[10px] text-muted-foreground px-1">
              {(() => {
            ...
            {currencyUnit === "USDT" && (
              <div className="flex gap-1">
                {(
                  ...
                  <button
                    key={m.value}
                    onClick={() => setUsdtInputMode(m.value)}
        ...
        {leverageExceeded && notionalValue > 0 && (
          <div className="flex items-start gap-1.5 px-2 py-1.5 rounded text-[10px] bg-destructive/10 text-destructive border border-destructive/20">
            ...
            <span>
              名义价值 {notionalValue.toFixed(0)} USDT 超出当前 {leverage}x 杠杆上限（最高 {maxAllowedLeverage}
              x），请降低杠杆或减小仓位
            </span>
          </div>
        )}
      ...
      {showTriggerSelector && (
        <BottomSheet title="选择触发类型" onClose={() => setShowTriggerSelector(false)}>
          ...
          >
            ...
            <p className="text-[11px] text-muted-foreground mt-1">标记价格为合约的预估公允价值，用于强制平仓计算。</p>
          </button>
          ...
          >
            ...
            <p className="text-[11px] text-muted-foreground mt-1">最新价格为该合约的最新成交价格</p>
          </button>
        </BottomSheet>
      )}
      ...
      {showCurrencySelector && (
        <BottomSheet title="货币单位" onClose={() => setShowCurrencySelector(false)}>
          ...
          >
            ...
            <p className="text-[11px] text-muted-foreground mt-1">输入并显示以 {baseCoin} 表示的订单金额。</p>
          </button>
          <div className="border-t border-border" />
          <button
            onClick={() => {
              ...
              <div className="flex gap-3 mt-2 ml-1">
                {[
                  ...
                  <label key={m.value} className="flex items-center gap-1.5 cursor-pointer">
                    ...
                    <span
                      className={`text-xs ${usdtInputMode === m.value ? "text-foreground" : "text-muted-foreground"}`}
                    >
                      {m.label}
                    </span>
                  </label>
