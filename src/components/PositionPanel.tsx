...
import { findUnreviewedJournalForClose, listJournals } from "@/lib/journalApi";
...
export function PositionPanel({
  ...
  const [adjustMarginModal, setAdjustMarginModal] = useState<{ symbol: string; index: number; pos: Position } | null>(
    null,
  );
  ...
  useEffect(() => {
    reloadJournals(); /* eslint-disable-next-line */
  }, [user]);
  ...
  const mergedPositions = useMemo(() => {
    ...
    for (const { symbol, position: pos, index } of displayedPositions) {
      ...
      g.weightedEntryPrice =
        (g.weightedEntryPrice * g.totalQuantity + pos.entryPrice * pos.quantity) / (g.totalQuantity + pos.quantity);
            ...
            return (
              <button
                ...
                className={`relative h-full text-sm font-medium whitespace-nowrap transition-colors ${
                  isActive ? "text-[#fcd535]" : "text-gray-500 dark:text-[#848e9c] hover:text-gray-900 dark:text-white"
                }`}
              >
                {labelText}
                {isActive && <span className="absolute left-0 right-0 -bottom-px h-[2px] bg-[#fcd535]" />}
              </button>
            );
          })}
        </div>
                        ...
                        >
                          <div className="flex items-center min-w-0">
                            ...
                            <button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                !col.locked && toggleColumn(col.key);
                              }}
                              disabled={col.locked}
                  ...
                  {mergedPositions.map((mg) => {
                    ...
                    const handleCloseGroup = (e: React.MouseEvent) => {
                      ...
                      if (mg.children.length === 1) {
                        ...
                        toast.success(
                          `已市价平仓 ${mg.children.length} 个 ${baseCoin} ${mg.side === "LONG" ? "多" : "空"}仓`,
                        );
                      }
                    };
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 px-3 py-2 border-b border-border/50">
                          ...
                          >
                            {mg.side === "LONG" ? "多" : "空"} {mg.leverage}x{" "}
                            {mg.marginMode === "cross" ? "全仓" : "逐仓"}
                          </span>
                        ...
                        <div className="flex items-start justify-between px-3 py-2.5">
                          <div>
                            ...
                            <div
                              className={`text-lg font-bold font-mono tabular-nums ${isProfit ? "text-trading-green" : "text-trading-red"}`}
                            >
                ...
                <tbody>
                  {allOrders.map(({ symbol, order }) => {
                    return (
                      <tr
                        key={order.id}
                        className="border-b border-gray-100 dark:border-[#2b3139]/50 hover:bg-gray-50 dark:hover:bg-white/5"
                      >
                        ...
                        <td
                          className={`px-3 py-2 font-bold ${order.side === "LONG" ? "text-[#0ecb81]" : "text-[#f6465d]"}`}
                        >
                          {order.side === "LONG" ? "多" : "空"}
                        </td>
                        ...
                        <td className="px-3 py-2 text-gray-900 dark:text-white">
                          {formatUSDT((order.price > 0 ? order.price : priceMap[symbol] || 0) * order.quantity)} USDT
                        </td>
              ...
              <table className="w-full text-[11px] font-mono tabular-nums">
                <thead className="sticky top-0 bg-white dark:bg-[#1e2329] z-10">
                  <tr className="text-gray-500 dark:text-[#848e9c] border-b border-gray-200 dark:border-[#2b3139]">
                    ...
                    <th
                      className="px-3 py-1.5 text-left font-medium whitespace-nowrap cursor-pointer select-none"
                  ...
                  {(() => {
                    ...
                    if (historySort === "pnl-desc") sorted.sort((a, b) => b.pnl - a.pnl);
                      ...
                      const pct = (t: (typeof sorted)[0]) => {
                        const m = (t.quantity * t.entryPrice) / t.leverage;
                        return m > 0 ? t.pnl / m : 0;
                      };
                    ...
                    <tr key={t.id} className="border-b border-gray-100 dark:border-[#2b3139]/50">
                      ...
                      <td className="px-3 py-2 text-gray-900 dark:text-white">
                        {t.exitPrice > 0 ? formatPrice(t.exitPrice, t.symbol) : "-"}
                      </td>
                      <td className="px-3 py-2 text-gray-900 dark:text-white">
                        {(t.quantity * t.entryPrice).toFixed(2)} USDT
                      </td>
                      <td className="px-3 py-2 text-gray-500 dark:text-[#848e9c]">
                        {t.openTime && t.openTime > 0
                          ? new Date(t.openTime).toLocaleString("zh-CN", {
                              month: "2-digit",
                              day: "2-digit",
                              hour: "2-digit",
                              minute: "2-digit",
                      ...
                      <td className="px-3 py-2 text-gray-500 dark:text-[#848e9c]">
                        {t.closeTime
                          ? new Date(t.closeTime).toLocaleString("zh-CN", {
                              month: "2-digit",
                              day: "2-digit",
                              hour: "2-digit",
                              minute: "2-digit",
                    ...
                    .map((t) => {
                      ...
                      return (
                        <tr key={t.id} className="border-b border-gray-100 dark:border-[#2b3139]/50">
                          ...
                          <td className="px-3 py-2 text-gray-900 dark:text-white">
                            {formatPrice(t.exitPrice > 0 ? t.exitPrice : t.entryPrice, t.symbol)}
                          </td>
                    ...
                    .map((t) => {
                      ...
                      return (
                        <tr key={t.id} className="border-b border-gray-100 dark:border-[#2b3139]/50">
                          ...
                          <td
                            className={`px-3 py-2 font-bold ${t.side === "LONG" ? "text-[#0ecb81]" : "text-[#f6465d]"}`}
                          >
                            {t.side === "LONG" ? "多" : "空"} {t.leverage}x
                          </td>
                          <td className="px-3 py-2 text-gray-900 dark:text-white">
                          ...
                          <td className="px-3 py-2 w-[80px]">
                            {!matchedJ ? (
                              ...
                              <button
                                onClick={() => {
                                  setReviewJournal(matchedJ);
                                  setReviewTradeRecord(t);
                                  setReviewOpen(true);
                                }}
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                  {cards.map((c) => {
                    ...
                    return (
                      ...
                      >
                        <div className="text-xs font-normal text-gray-500 dark:text-[#848e9c] mb-1.5 whitespace-nowrap overflow-hidden text-ellipsis">
                          {c.label}
                        </div>
                        <div className="flex items-baseline space-x-1.5">
                          <span className={`font-mono text-xl font-semibold tracking-tight tabular-nums ${color}`}>
                            {display}
                          </span>
