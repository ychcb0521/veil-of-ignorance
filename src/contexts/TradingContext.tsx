...
export function TradingProvider({ children }: { children: React.ReactNode }) {
    ...
    for (const [sym, positions] of Object.entries(positionsMap)) {
      ...
      for (const pos of positions) {
        ...
        fundingRecords.push({
          ...
          margin: pos.marginMode === "isolated" && pos.isolatedMargin != null ? pos.isolatedMargin : pos.margin,
          pnl: amount,
          fee: Math.abs(fee),
          slippage: 0,
          openTime: now,
  ...
  const handlePlaceOrder = useCallback(
    (symbol: string, order: PlaceOrderParams) => {
  ...
  const handleClosePosition = useCallback(
    (symbol: string, index: number, percentage: number = 1) => {
      ...
      setTradeHistory((prev) => [
        ...prev,
        {
          ...
          leverage: pos.leverage,
          marginMode: pos.marginMode,
          margin: pos.marginMode === "isolated" && closedIsoMargin != null ? closedIsoMargin : closedMargin,
          pnl: pnl - fee,
          fee,
          slippage,
          openTime: pos.openTime,
